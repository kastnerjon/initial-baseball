...
export function createSupabaseDailyPuzzleRepository(
  ...
  return {
    ...
    async save(record, options) {
      validateRevisionTransition(record, options);
...
function encodeDailyPuzzleInsert(record: DailyPuzzleEditorialRecord): Record<string, unknown> {
  return {
    id: record.id,
...
function encodeDailyPuzzleUpdate(record: DailyPuzzleEditorialRecord): Record<string, unknown> {
  return {
    revision: record.revision,
    status: record.status,
    selections: record.selections.map(selection => ({ ...selection })),
...
function decodeDailyPuzzleRow(value: unknown): DailyPuzzleEditorialRecord {
