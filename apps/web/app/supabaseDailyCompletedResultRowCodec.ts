...
): Omit<DailyCompletedResultRow, 'created_at'> {
  return {
...
export function decodeDailyCompletedResultRow(row: unknown): DailyCompletedResult {
  ...
  const puzzleDate = requireDateString(row.puzzle_date);
  ...
  if (row.ruleset_version === POINTS_V3_DAILY_RULESET_VERSION) {
    const summary = requirePointsSummary(row.summary);
    return {
      schemaVersion,
      submissionId,
      puzzleId,
  ...
  if (row.ruleset_version === CLASSIC_DAILY_RULESET_VERSION) {
    const summary = requireClassicSummary(row.summary);
    return {
      schemaVersion,
      submissionId,
      puzzleId,
...
function requireCompletedAtBat(value: unknown, index: number): DailyCompletedAtBat {
  ...
  const pitchNumber = requirePositiveInteger(value.pitchNumber, `completed_at_bats[${index}].pitchNumber`);
  const initials = requireNonEmptyString(value.initials, `completed_at_bats[${index}].initials`);
  const outcome = requireOutcome(value.outcome, index);
  const hintsRevealed = requireRevealCount(value.hintsRevealed, index);
  const wrongGuesses = requireIntegerWithin(value.wrongGuesses, 0, 3, `completed_at_bats[${index}].wrongGuesses`);
  if (value.resolution !== 'correct' && value.resolution !== 'strikeout' && value.resolution !== 'give_up') {
...
function requirePointsSummary(value: unknown) {
  ...
  return {
    ...
    atBatsCompleted: requireNonNegativeInteger(value.atBatsCompleted, 'summary.atBatsCompleted'),
...
function requireOutcome(value: unknown, index: number): DailyOutcome {
  if (value !== 'HR' && value !== '3B' && value !== '2B' && value !== '1B' && value !== 'BB' && value !== 'K') {
...
function requireRevealCount(value: unknown, index: number): DailyRevealCount {
  return requireIntegerWithin(value, 0, 4, `completed_at_bats[${index}].hintsRevealed`) as DailyRevealCount;
}
...
function requireIntegerWithin(value: unknown, minimum: number, maximum: number, field: string): number {
