...
export const DAILY_STORAGE_SCHEMA_VERSION = 3 as const;
export const MAX_DAILY_PROGRESSION_TOKEN_LENGTH = 4096;
...
export function toPublicDailyPuzzle(
...
export function decodeSavedDailyGame(
  value: unknown,
  puzzle: DailyPublicPuzzle | DailyPuzzle,
  initialProgressionToken: string,
): LoadedSavedDailyGame | null {
  const publicPuzzle = toPublicDailyPuzzle(puzzle);
  if (!isSavedDailyGameForPuzzle(value, publicPuzzle)) return null;

  try {
    const savedGame = normalizeSavedDailyGame(value, publicPuzzle, initialProgressionToken);
    if (savedGame === null) return null;

    return {
      savedGame,
      completedAtBatFactsAreNative: value.schemaVersion === DAILY_STORAGE_SCHEMA_VERSION
        && isNativeCompletedAtBatList(
          value.gameState.completedAtBats,
          value.gameState.completedPitchLines,
        )
        && (
          value.pendingAdvance === null
          || isNativeCompletedAtBatList(
            value.pendingAdvance.completedAtBats,
            value.pendingAdvance.pitchLines,
          )
        ),
    };
  } catch {
    // Browser-local persistence is untrusted input. Corruption must not break bootstrap.
    return null;
  }
}
...
function isSavedDailyGameForPuzzle(
  ...
  if (
    ...
    || (value.schemaVersion === DAILY_STORAGE_SCHEMA_VERSION
      && !isValidProgressionToken(value.progressionToken))
  ) {
    return false;
  }

  return hasNormalizableGameState(value.gameState, puzzle)
    && hasNormalizableAtBatState(value.atBatState)
    && hasNormalizablePendingAdvance(value.pendingAdvance);
}

function hasNormalizableGameState(
  gameState: Record<string, unknown>,
  puzzle: DailyPublicPuzzle,
): boolean {
  return isRecord(gameState.puzzle)
    ...
    && typeof gameState.score.completed === 'boolean'
    && Array.isArray(gameState.completedPitchLines)
    && gameState.completedPitchLines.every(isNormalizableSharePitchLine)
    && hasNormalizableShareResult(gameState.shareResult);
}

function hasNormalizableAtBatState(atBatState: Record<string, unknown>): boolean {
  return typeof atBatState.query === 'string'
    ...
    && (atBatState.revealedHints === undefined || Array.isArray(atBatState.revealedHints))
    && hasNormalizableGuessResult(atBatState.submittedResult);
}

function hasNormalizablePendingAdvance(value: unknown): boolean {
  if (value === null) return true;
  if (!isRecord(value)) return false;

  return isRecord(value.inning)
    && isRecord(value.score)
    && typeof value.score.completed === 'boolean'
    && Array.isArray(value.pitchLines)
    && value.pitchLines.every(isNormalizableSharePitchLine)
    && typeof value.nextPitchIndex === 'number';
}

function hasNormalizableShareResult(value: unknown): boolean {
  if (value === null) return true;
...
function hasNormalizableGuessResult(value: unknown): boolean {
  if (value === null) return true;
  if (!isRecord(value)) return false;

  if (value.kind === 'incorrect') {
    return typeof value.revealedCount === 'number'
      && typeof value.strikeCount === 'number'
      && typeof value.remainingStrikes === 'number';
  }

  if (value.kind === 'strikeout') {
    return typeof value.revealedCount === 'number'
      && typeof value.strikeCount === 'number'
      && value.outcome === 'K'
      && value.source === 'strikeout';
  }

  return value.kind === 'correct'
    && typeof value.revealedCount === 'number'
    && isNormalizableCorrectOutcome(value.outcome);
}

function isNormalizableSharePitchLine(value: unknown): boolean {
  return isRecord(value)
    && typeof value.initials === 'string'
    && isNormalizableOutcome(value.outcome);
}

function isNormalizableCorrectOutcome(value: unknown): boolean {
  return value === 'HR'
    || value === '3B'
    || value === '2B'
    || value === '1B'
...
function isNormalizableOutcome(value: unknown): boolean {
  return isNormalizableCorrectOutcome(value) || value === 'K';
}
...
function normalizeRulesetVersion(
  ...
  if (isDailyRulesetVersion(value)) return value;
...
function isValidProgressionToken(value: unknown): value is string {
  return typeof value === 'string'
    && value.length > 0
    && value.length <= MAX_DAILY_PROGRESSION_TOKEN_LENGTH;
}
