...
describe('Daily Nine comparison HTTP mapping', () => {
  it('keeps public reads disabled unless explicitly activated', async () => {
    expect(isDailyNineComparisonReadEnabled({})).toBe(false);
    expect(isDailyNineComparisonReadEnabled({ DAILY_NINE_COMPARISON_READS_ENABLED: 'false' })).toBe(false);
    expect(isDailyNineComparisonReadEnabled({ DAILY_NINE_COMPARISON_READS_ENABLED: ' TRUE ' })).toBe(true);
  ...
  it.each([
    ...
    [new DailyRuntimeRequestError('hidden puzzle detail'), 503, 'comparison_unavailable'],
