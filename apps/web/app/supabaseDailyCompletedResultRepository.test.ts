...
describe('Supabase completed-result repository', () => {
  it('inserts the complete normalized result without an update/upsert path', async () => {
  ...
  it('fails closed if a conflicted provider row is malformed', async () => {
    const insertSingle = vi.fn().mockResolvedValue({
      data: null,
      error: { code: '23505', message: 'duplicate key value' },
    });
    ...
    const readSelect = vi.fn().mockReturnValue({
    ...
    await expect(createSupabaseDailyCompletedResultRepository(asClient(from))
      .insertIfAbsent(RESULT)).rejects.toMatchObject({ kind: 'invalid-row' });
  });
  ...
  it('maps non-unique provider failures to a query error', async () => {
    ...
    await expect(createSupabaseDailyCompletedResultRepository(asClient(
      vi.fn().mockReturnValue({ insert }),
    )).insertIfAbsent(RESULT)).rejects.toMatchObject({
      kind: 'query',
  ...
  it('treats a unique conflict with no readable winner as a provider invariant failure', async () => {
    ...
    await expect(createSupabaseDailyCompletedResultRepository(asClient(from))
      .insertIfAbsent(RESULT)).rejects.toMatchObject({ kind: 'query' });
  });
});
