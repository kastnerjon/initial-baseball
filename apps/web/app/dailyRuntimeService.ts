...
export function createDailyRuntimeService({ canonicalRuntime, createPuzzle, progressionTokens }: CreateDailyRuntimeServiceInput): DailyRuntimeService {
  async function createCanonicalPuzzle(date: string): Promise<DailyPuzzle> {
    ...
    return {
      ...
      pitches: puzzle.pitches.map(pitch => ({
        ...pitch,
        player: { ...pitch.player, playerId: canonicalRuntime.requireCanonicalPlayerId(pitch.player.playerId) },
      })),
    };
  }
  ...
  async function requireAuthorizedProgression(progressionToken: string): Promise<AuthorizedProgression> {
    ...
    try {
      ...
      if (error instanceof DailyProgressionTokenError) throw new DailyRuntimeRequestError('Invalid Daily progression token.');
      throw error;
    }
    if (claims.completed) throw new DailyRuntimeRequestError('This Daily progression token is already complete.');
  ...
  return {
    async getBootstrap(date) {
      ...
      if (firstPitch === undefined) throw new DailyRuntimeRequestError(`Daily puzzle ${puzzle.id} has no pitches.`);
      return {
    ...
    async revealHint(progressionToken) {
      const { claims, puzzle, pitch } = await requireAuthorizedProgression(progressionToken);
      const hintSlot = puzzle.hintConfig[claims.revealCount];
      if (hintSlot === undefined) throw new DailyRuntimeRequestError(`No additional hint exists for pitch ${claims.pitchNumber}.`);
      const hintValue = pitch.hints[hintSlot.hintType];
      if (hintValue === undefined) throw new DailyRuntimeRequestError(`Hint ${hintSlot.hintType} is unavailable for pitch ${claims.pitchNumber}.`);
      return {
        hint: { hintType: hintSlot.hintType, hintLabel: hintSlot.displayLabel, hintValue },
        progressionToken: progressionTokens.sign({ ...claims, revealCount: incrementRevealCount(claims.revealCount) }),
      };
    },
    ...
    async resolveAtBat(request) {
      ...
      const result = request.giveUp === true
        ? { kind: 'strikeout' as const, revealedCount: authorized.claims.revealCount, strikeCount: 3, outcome: 'K' as const, source: 'strikeout' as const }
        : getGuessOutcome({ isCorrect, revealCount: authorized.claims.revealCount, strikeCount: authorized.claims.strikeCount, maxStrikes: 3 });
      const isTerminal = result.kind === 'correct' || result.kind === 'strikeout';
      return {
        result,
        reveal: isTerminal ? createCanonicalRevealViewModel(canonicalRuntime.getReveal(authorized.pitch.player.playerId)) : null,
...
export function toPublicPuzzle(puzzle: DailyPuzzle): DailyPublicPuzzle {
  return {
    id: puzzle.id, puzzleNumber: puzzle.puzzleNumber, puzzleDate: puzzle.puzzleDate, status: puzzle.status,
    hintConfig: puzzle.hintConfig, statsHintConfig: puzzle.statsHintConfig,
    pitches: puzzle.pitches.map(pitch => ({ pitchNumber: pitch.pitchNumber, initials: pitch.player.initials })),
  };
}

function createSuccessorClaims(authorized: AuthorizedProgression, result: DailyGuessResult): DailyProgressionClaims {
  if (result.kind === 'incorrect') return { ...authorized.claims, strikeCount: result.strikeCount as 1 | 2 };
...
function requirePitch(puzzle: DailyPuzzle, pitchNumber: number): DailyPuzzle['pitches'][number] {
  ...
  if (pitch === undefined) throw new DailyRuntimeRequestError(`Unknown pitch ${pitchNumber} for ${puzzle.puzzleDate}.`);
  return pitch;
}

function resolveSubmittedPlayerId(request: DailyResolutionRequest, canonicalRuntime: CanonicalRuntimeAccessor): string | null {
  if (request.giveUp === true) return null;
  const submittedPlayerId = request.submittedPlayerId?.trim();
  if (!submittedPlayerId) throw new DailyRuntimeRequestError('submittedPlayerId is required for a guess.');
  try { return canonicalRuntime.requireCanonicalPlayerId(submittedPlayerId); }
  catch { throw new DailyRuntimeRequestError('submittedPlayerId does not resolve to a canonical player.'); }
}
