-- Daily editorial persistence for the provider-neutral DailyPuzzleRepository.
...
create table if not exists public.daily_puzzles (
  ...
  constraint daily_puzzles_lifecycle_audit_check check (
    (
      status = 'draft'
...
comment on table public.daily_puzzles is
  'Canonical-ID-only Daily editorial records governed by packages/daily lifecycle rules.';
comment on column public.daily_puzzles.selections is
  'Exactly nine {slot, canonicalPlayerId, source} records; baseball facts are joined at read time.';
comment on column public.daily_puzzles.revision is
...
grant select, insert, update on table public.daily_puzzles to service_role;
