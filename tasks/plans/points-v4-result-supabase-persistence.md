...
- `daily_at_bat_results`: v3 `0..7`; v4 `-1..4`;
- `daily_completed_results.summary.points`: integer v3 `0..63`; integer v4 `-9..36`;
- Classic completed rows bypass the points-summary range check.

The ruleset discriminator is part of each constraint, so widening v4 cannot silently make negative v3 rows valid.
