...
13. Current: portable comparison HTTP response contract in `packages/shared`. Version success/error payloads, preserve exact puzzle/ruleset/slot identity, and expose `sourceReadAt` / live-vs-cached metadata without implementing caching. Scope: `tasks/plans/daily-nine-comparison-api-contract.md`.
