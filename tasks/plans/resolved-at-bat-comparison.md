...
15. Current: thin default-off Next.js GET adapters (PR #201). Keep at-bat/completed routes separate, sanitize HTTP errors, use `private, no-store`, and require server-only `DAILY_NINE_COMPARISON_READS_ENABLED=true`; do not enable it in deployment config. Scope: `tasks/plans/daily-nine-comparison-read-api.md`.
16. Next: representative isolated performance evidence and any measured cache/rollup decision as separate scope if needed, then an explicit activation decision.
