...
13. Current: portable comparison HTTP response contract in `packages/shared`. Version success/error payloads, preserve exact puzzle/ruleset/slot identity, and expose `sourceReadAt` / `servedAt` / live-vs-cached metadata without implementing caching. Scope: `tasks/plans/daily-nine-comparison-api-contract.md`.
14. Next: web/server read-only API routes and authoritative request composition only. Keep comparison failure separate from write acknowledgment and do not add React or cache policy.
15. Then: representative isolated performance evidence and any measured cache/rollup decision as separate scope if needed.
