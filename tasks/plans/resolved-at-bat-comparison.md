...
24. Issue #216 is corrected after PR #229: scorecard/share AVG is per AB, not an extra whole-game scorecard metric. Ongoing/completed private rows asynchronously read the existing exact-pitch aggregate, with 0–1 withheld and 2+ shown as ordinary one-decimal `AVG x.x`; spoiler-safe share pitch lines receive the same per-AB value. An ephemeral per-pitch web cache refills current aggregates on restore and allows low-sample/unavailable rows to retry as later ABs complete. The separate completed-game YOU / AVG / BEAT panel remains unchanged. Onset-of-AB display remains a separate presentation-policy decision.
