...
14. Current: web/server read-only API routes and authoritative request composition. `GET /api/daily/comparison/at-bat` accepts date/ruleset/pitch; `GET /api/daily/comparison/completed` accepts date/ruleset. The server derives puzzle ID/number from the authoritative public puzzle, stamps live source freshness, returns `no-store`, and keeps read failures separate from write acknowledgment. Scope: `tasks/plans/daily-nine-comparison-read-routes.md`.
15. Next: representative isolated performance evidence and any measured cache/rollup decision as separate scope if needed.
