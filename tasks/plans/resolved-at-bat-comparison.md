...
14. Current: server-side comparison read composition (PR #200). Validate date/ruleset/pitch, derive puzzle ID/number from the authoritative public puzzle, compose the existing Daily comparison service with the Supabase provider, and capture conservative live source freshness. No public route is added. Scope: `tasks/plans/daily-nine-comparison-server-read.md`.
15. Next: thin Next.js read-route adapters, sanitized HTTP mapping, `no-store`, and a default-off activation gate. Then gather representative isolated performance evidence before production activation.
16. Then: measured cache/rollup decision if needed, followed by explicit route activation.
17. Reveal/final-scorecard UI: asynchronous YOU / AVG after every AB, low samples/outages, final refresh, stale-response protection, mobile and answer-integrity verification.
