...
15. Current: thin default-off Next.js GET adapters (PR #202). Keep at-bat/completed routes separate, sanitize HTTP errors, use `private, no-store`, and require server-only `DAILY_NINE_COMPARISON_READS_ENABLED=true`; do not enable it in deployment config. Scope: `tasks/plans/daily-nine-comparison-read-api.md`.
