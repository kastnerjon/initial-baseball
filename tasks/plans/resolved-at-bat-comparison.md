...
24. Daily Nine result presentation is points-native end to end: terminal result shows engine-derived `N PTS`; private/share per-AB comparison is personal AB points versus crowd AVG; completed private/share header is `X PTS • AVG Y.Y` when displayable, with existing BEAT/sample context secondary. Baseball outcomes remain native stored/game facts but are not presented as the user's Daily Nine score because points-v3 is not a one-to-one outcome mapping. Personal AB points come from stored terminal facts through the existing engine rule; crowd values come from existing exact-pitch/completed comparison reads. 0–1 averages remain withheld and Classic remains baseball-native. Onset-of-AB AVG remains a separate presentation-policy decision.
