...
14. Current: web/server read-only API routes and authoritative request composition. `GET /api/daily/comparison/at-bat` accepts date/ruleset/pitch; `GET /api/daily/comparison/completed` accepts date/ruleset. The server derives puzzle ID/number from the authoritative public puzzle, captures conservative live source freshness, returns `no-store`, and keeps read failures separate from write acknowledgment. The routes are dark by default behind `DAILY_NINE_COMPARISON_READS_ENABLED=true`, so merge does not activate unbenchmarked reads. Scope: `tasks/plans/daily-nine-comparison-read-routes.md`.
15. Next: representative isolated performance evidence and any measured cache/rollup decision, followed by an explicit read-route activation decision.
16. Reveal/final-scorecard UI: asynchronous YOU / AVG after every AB, low samples/outages, final refresh, stale-response protection, mobile and answer-integrity verification.
