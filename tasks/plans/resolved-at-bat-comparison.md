...
24. Issue #216 is corrected through PRs #229–#230 and the final scorecard presentation follow-up: the metric is per AB, and the private/share comparison is personal AB **points versus crowd AVG**, not baseball outcome versus AVG. Personal points are derived from stored completed-at-bat facts by the existing engine scoring rule, while the crowd side comes from the existing exact-pitch comparison read. Daily Nine private rows keep initials, answer, personal points and AVG collinear; spoiler-safe share rows use `BB: 0 • AVG: 7.0`; 0–1 comparison observations withhold AVG. Classic retains baseball outcomes. The separate completed-game YOU / AVG / BEAT panel remains unchanged. Onset-of-AB display remains a separate presentation-policy decision.
