...
Status: Approved direction; collection and R1–R4 browser prerequisites complete; comparison GET reads and Daily Nine browser consumers implemented; #227 hosted/source verification recorded, ordinary-browser/mobile verification remains
Date: 2026-09-18
...
- After saved-game hydration, quietly prefetch the exact active Daily Nine AB comparison for points-v3, but never display it before resolution. After every terminal AB (correct, strikeout or Give Up), show the normal answer/outcome/engine points immediately and reveal YOU / AVG from the existing prefetched/in-flight read state. Next At Bat never waits.
...
- Starting the AB read before resolution normally places it before the current result write, but immediate self-exclusion is still not an atomic guarantee: a slow managed read can remain in flight while the independent result write lands. A separately restored terminal page may also fetch the current aggregate. Historical comparison snapshots are not persisted. Whole-game comparison retains its existing current-population semantics. Return freshness metadata; cached data and failed reads are distinguishable.
...
18. Terminal-at-bat browser checkpoint implemented: a dedicated identity-keyed hook uses the PR #206 client + PR #207 controller, shows asynchronous YOU / AVG with 0–1 waiting / 2–9 early / 10+ normal states, degrades read failure quietly, derives YOU from the already-computed engine transition, and fences advance/reset/restore without touching result-write delivery.
19. Completed-game browser checkpoint implemented: a separate completed-channel hook starts from the final pending points-v3 engine summary, keeps the same request through View Results when semantic identity is unchanged, refreshes current comparison on restored completion, reuses Daily-owned strict-lower tie semantics, withholds BEAT until 20 completions, and keeps comparison failure isolated from final results/share/completion delivery.
20. September 20 hosted platform checkpoint recorded without runtime changes: exact-main CI #771 is green, exact-main production deployment `dpl_6N5RxYheUVBEi89NZSmJm1ynz88a` is READY, current comparison route logs show HTTP 200 reads with no runtime errors in the reviewed window, Supabase comparison RPC access remains server-only, and exact-main build/hidden-answer evidence is recorded. Evidence: `docs/engineering/daily-nine-comparison-hosted-verification-2026-09-20.md`.
21. September 22 provider evidence closes storage/query diagnosis for this beta checkpoint: provider setup/decode are negligible, multi-second tails track awaited `client.rpc(...)`, and matching PostgreSQL execution remains low-millisecond. No SQL/index/rollup/cache change is justified.
22. Hidden active-at-bat prefetch is implemented in the web consumer: the exact hydrated points-v3 slot starts reading before resolution, public comparison state remains idle until own points exist, and the terminal transition projects the same read instead of starting another request.
23. PR #227 post-merge hosted/source verification is recorded in `docs/engineering/daily-nine-comparison-prefetch-verification-2026-09-22.md`: exact main/CI/production identity, aliases, public route health, runtime error/fatal logs, source wiring and existing automated stale-response/hidden-presentation coverage are verified. An ordinary-browser network trace, browser trigger-to-visible timing, controlled delayed/failed rendered behavior and physical/mobile QA remain open; do not claim those from source inspection.
24. Daily Nine result presentation is points-native end to end: terminal result shows engine-derived `N PTS`; private/share per-AB comparison is rendered from one shared initials / SCORE / AVG row model; completed private/share header is `X PTS • AVG Y.Y` when displayable, with existing BEAT/sample context secondary. Baseball outcomes remain native stored/game facts but are not presented as the user's Daily Nine score because points-v3 is not a one-to-one outcome mapping. Personal AB points come from stored terminal facts through the existing engine rule; crowd values come from existing exact-pitch/completed comparison reads. Player names are intentionally omitted from the scorecard table and remain in the reveal experience; missing AVG renders `—`. 0–1 averages remain withheld and Classic remains baseball-native. Onset-of-AB AVG remains a separate presentation-policy decision.
