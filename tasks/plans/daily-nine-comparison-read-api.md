...
The separate activation scope is `tasks/plans/daily-nine-comparison-read-activation.md`. It did not rewrite these route contracts. PR #204 merged the post-review default-on model with server-only fail-closed `DAILY_NINE_COMPARISON_READS_DISABLED` as the emergency switch and is production-verified on merge `178e58cb6fcb8f09ad9ebc3e6ba69cca7a725a01` / deployment `dpl_F87hcE51cQT6zhoCp8MgGKwCSX38`; both GET routes returned versioned HTTP 200 live payloads with `private, no-store`, and the comparison-route runtime scan was clean. Browser/UI consumption remains separate and not yet implemented.
