...
2. **R2 gameplay requests — implemented.** A small web request controller provides synchronous single-flight request epochs. Reset, durable restore, session identity changes and non-owner access invalidate the active epoch; response parsing, success, error and finally work all require the exact current token. Old cleanup cannot clear a newer request. The shared component keeps Daily Nine and Classic behavior aligned; scoring and AB delivery are unchanged.
