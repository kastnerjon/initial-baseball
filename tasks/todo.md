...
- [x] Issue #216 corrected requirement: show the existing exact-pitch Daily Nine AVG on each resolved row of the ongoing/completed private scorecard and on the corresponding spoiler-safe share line; 0–1 withholds, 2+ displays ordinary one-decimal `AVG x.x`, restore refills current aggregates, and no answer/raw participant data enters share output. PR #229's extra whole-game scorecard/share decoration is superseded; the separate completed-game YOU / AVG / BEAT panel remains. Scope: `tasks/plans/daily-nine-scorecard-average.md`.
