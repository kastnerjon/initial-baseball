...
- [ ] Before permanent issuance, make archived clue/hint behavior immutable alongside frozen canonical player IDs; treat this as a separate portable-contract/persistence decision. Portable clue snapshot contract is complete: one four-slot public layout plus exact ordered canonical IDs, initials, and four hint values are modeled without freezing scoring. Next: append-only persistence/codec migration, then issuance/materialization wiring. Scope: `tasks/plans/permanent-daily-clue-snapshot-contract.md`. After the full immutability path is complete, omit terminal Jr/Sr from generated initials without altering player names/aliases or silently mixing same-identity beta observations during rollout.
