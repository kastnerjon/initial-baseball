...
- [x] Define `points-v4` in shared/engine with HR 4 / 3B 3 / 2B 2 / 1B 1 / BB 0 / K or Give Up -1, no deduction for wrong guesses one or two, signed -9..36 nine-AB range, and preserved points-v3 default/history. No result/browser/live activation. Scope: `tasks/plans/points-v4-engine-ruleset.md`.
...
- [x] Make the portable Daily comparison domain version-aware for v3/v4, including v4 -1..4 AB observations, -9..36 completed totals, offset histogram semantics, strict-lower beat calculations, and preserved v3 provider-read boundaries. Scope: `tasks/plans/points-v4-comparison-domain.md`.
...
- [x] G3B: widen the comparison RPCs plus Supabase comparison decoder and Daily repository/service read port for signed v4 aggregates, preserving v3 reads, service-role-only function access, and the still-v3 public HTTP/browser boundary. Scope: `tasks/plans/points-v4-comparison-supabase-provider.md`.
- [x] H1: widen only the server completed-result and resolved-AB write preflights to exact-version `points-v4`, reusing the existing engine validation and G3A storage while leaving browser delivery, comparison HTTP/UI and the public default on v3. Scope: `tasks/plans/points-v4-result-write-api.md`.
- [ ] Extend browser persistence/delivery/scorecards/comparison presentation for v4 and perform a separate safe public-default cutover without migrating v3 saves. Later BB 0.5 / K 0 would be a different version.
