...
- [ ] Before archive exposure, make archived clue/hint behavior immutable alongside frozen canonical player IDs; the portable clue contract, schema-v2 envelope, append-only Supabase/codec support, portable v2 issuance, and server v2 clue materialization are now complete without freezing scoring. Next: teach archive materialization to consume persisted v2 clues instead of rebuilding mutable clues; keep schema-v1 backward reads safe. Scopes: `tasks/plans/permanent-daily-clue-snapshot-contract.md`, `tasks/plans/permanent-daily-issued-puzzle-v2-contract.md`, `tasks/plans/permanent-daily-issued-puzzle-v2-supabase.md`, and `tasks/plans/permanent-daily-server-clue-issuance.md`. After the full immutability path is complete, omit terminal Jr/Sr from generated initials without altering player names/aliases or silently mixing same-identity beta observations during rollout.
