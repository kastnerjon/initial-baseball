...
- [x] Before archive exposure, make archived clue/hint behavior immutable alongside frozen canonical player IDs; the portable clue contract, schema-v2 envelope, append-only Supabase/codec support, portable v2 issuance, server v2 clue materialization, and archive v2 clue consumption are complete without freezing scoring. Schema-v1 archive rows remain readable. Scope: `tasks/plans/permanent-daily-archive-clue-materialization.md`. Next on the clue track: omit terminal Jr/Sr from generated initials without altering player names/aliases or silently mixing same-identity beta observations during rollout.
