...
- [x] Issue #216: show the existing whole-game Daily Nine comparison AVG on both the normal completed scorecard and spoiler-safe share output using the same completed-comparison state/sample semantics; 0–1 withholds AVG, 2–9 labels Early AVG, 10+ labels AVG, and share output adds no per-AB averages/answers/raw data. Scope: `tasks/plans/daily-nine-scorecard-average.md`.
