...
- [x] Issue #216 final corrected requirement: Daily Nine scorecard/share compares the user's engine-derived points for each resolved AB against that exact pitch's crowd AVG. Private row: `BB | Barry Bonds | 0 | AVG 7.0`; spoiler-safe share row: `BB: 0 • AVG: 7.0`. 0–1 comparison observations withhold AVG; refresh/restore recomputes personal points from persisted completed-at-bat facts through the existing engine rule; Classic keeps baseball outcomes; mobile private rows stay single-line. The separate completed-game YOU / AVG / BEAT panel remains. Scope: `tasks/plans/daily-nine-scorecard-average.md`.
