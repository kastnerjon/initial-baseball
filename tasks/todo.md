...
- [ ] Before permanent issuance, make archived clue/hint behavior immutable alongside frozen canonical player IDs; treat this as a separate portable-contract/persistence decision. Portable clue snapshot contract, schema-v2 envelope, and append-only Supabase/codec support for v1 + v2 are complete without freezing scoring. Portable v2 issuance is complete. Next: cut server issuance composition over to v2 using the exact current public clue materialization, then wire archive materialization to consume frozen clues. Scopes: `tasks/plans/permanent-daily-clue-snapshot-contract.md`, `tasks/plans/permanent-daily-issued-puzzle-v2-contract.md`, and `tasks/plans/permanent-daily-issued-puzzle-v2-supabase.md`. After the full immutability path is complete, omit terminal Jr/Sr from generated initials without altering player names/aliases or silently mixing same-identity beta observations during rollout.
