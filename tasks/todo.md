...
- [ ] Decide later whether per-AB AVG should remain terminal-only or be prefetched/shown at AB onset. The current puzzle + ruleset + slot read contract supports either; treat this as presentation policy because showing the crowd average before play may affect player behavior.
...
- [ ] Re-sample production using provider sub-timing and identify whether the long tail is RPC wait versus local setup/decode before changing provider behavior.
- [ ] Measure real browser trigger-to-visible comparison latency against the provisional end-to-end target after the browser consumer is live.
