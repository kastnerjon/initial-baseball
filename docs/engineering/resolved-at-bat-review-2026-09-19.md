...
The later R7 checkpoint remains `DailyInningGame` orchestration decomposition. The earlier prerequisites are already in place: R2 extracted gameplay request lifetime, R1/R3 made owner authority explicit, and comparison reads live in separate identity-keyed client/controller/hooks and small presentation components. Extract only cohesive responsibilities with concrete current consumers, keep gameplay rules in engine/Daily, keep comparison independent from persistence/write delivery, and do not introduce a generic repository/outbox/framework solely to reduce line count. No current correctness defect is asserted by R7; the goal is to make lifecycle ownership easier to reason about before the next feature adds pressure.
