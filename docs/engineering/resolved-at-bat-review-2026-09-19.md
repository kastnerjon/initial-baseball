...
At the initial September 20 R7 review, `DailyInningGame.tsx` was 495 lines and combined view decisions, coordinated state setters, resolution transport, reset, hint hydration and composition of comparison/persistence hooks; `useDailyGameplayPersistence.ts` was 433 lines and concentrated lock ownership, eligibility, restore readiness, saves, freezes, delivery recovery, completion creation and degradation. R7A–R7C reduce those concentrations without turning line count into the goal: after R7C, `DailyInningGame.tsx` is 411 lines and retains gameplay state/render/action coordination while the independently stateful browser lifecycles have concrete owners.

R7 is deliberately split rather than forcing one large refactor. The first checkpoint extracts the mutable persistence owner-session mechanics — current semantic session/readiness/access, contribution lifetime, exact owner-delivery lifetime, completion-creation identity, and stale failure fencing — into a concrete web-only `dailyGameplayPersistenceSession` controller with deterministic tests. `useDailyGameplayPersistence` remains the sole React effect and ordering authority: it still owns durable restore/authority composition and the save → freeze → asynchronous delivery sequence. This is not a new persistence framework and does not change storage, scoring, comparison, Web Lock or delivery semantics.

R7B extracts the saved-game restore plus authorized-hint hydration lifecycle from `DailyInningGame` into a concrete web-only `dailySavedGameRestoreController`. The controller owns restore generations, bootstrap-bundle reuse, completed-save no-bundle handling, active-save hint hydration, and stale success/failure/settled fencing; the component still owns gameplay state, comparison invalidation, rendering and action dispatch. R7C then performs the final justified extraction: `/api/daily/resolve` transport, single-flight fencing, pending-action lifecycle and generic request failure move behind a concrete Daily-specific web client/hook that composes the existing request controller. Fresh-eye review found that persistence-session invalidation previously revoked request authority without clearing `pendingResolutionAction`, while the stale request's `finally` was correctly suppressed; that combination could leave the UI stuck pending. R7C makes invalidation clear pending UI synchronously while late success/error/settled work remains inert. Terminal game transitions stay in the existing engine/Daily seam and component action coordination; no generic state-machine rewrite is justified. R7 is structurally complete after this checkpoint.
...
R8 is intentionally decomposed rather than implemented as one generic hardening PR. R8A owns only bounded body admission: both anonymous result-write routes now use one concrete web ingress reader with a 16 KiB actual-byte ceiling before JSON parsing, a Content-Length fast reject that is never trusted as sole enforcement, preserved 400 malformed-JSON behavior, and terminal 413 handling for oversized bodies. Current first-party schema-1 payloads are roughly 0.4 KiB for a padded single-AB observation and 1.3 KiB for a padded nine-AB completion, leaving substantial headroom. R8B remains a separate hosting-specific coarse rate-admission checkpoint; independent follow-up review places it at the Vercel WAF boundary rather than in app code. The target is only POST `/api/daily/at-bats` plus POST `/api/daily/results`, counted by client IP with a fixed window. Begin with a provisional 300 requests per 600 seconds and over-limit action `log`; review real traffic and shared-egress risk before any 429 enforcement, verify enforcement in Preview first, and keep the threshold reversible. No `@vercel/firewall`, database counter, cache, or new persistence primitive is justified for this coarse anonymous-IP control. R8C is implemented separately at the result-write HTTP error boundary: only caught internal failures mapped to 503/500 emit one low-cardinality JSON event (`daily_result_write_failure`) with route, coarse category and status. The mapper classifies provider configuration, provider repository and unexpected failures without passing the raw exception to the diagnostic helper; success, expected 4xx paths and authoritative-puzzle request errors remain silent. Public responses stay unchanged. No payload, answer/hint data, credential, raw Supabase detail, IP, user agent, attempt/submission ID, stack trace or persistent browser identifier enters the event. None of the three R8 checkpoints is evidence of honest human play.
...
Review completion means evidence and next work are recorded. R1–R6 have bounded repairs and R7 is structurally complete through the bounded R7A/R7B/R7C browser checkpoints. R8A bounded body admission and R8C sanitized handled-failure diagnostics are implemented. R8B external WAF publication/observation/enforcement and the remaining interactive/mobile verification are still open.
