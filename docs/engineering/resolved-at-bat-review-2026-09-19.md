...
At the September 20 checkpoint, `DailyInningGame.tsx` is 495 lines, combining view decisions, coordinated state setters, resolution transport, reset, hint hydration and composition of the comparison/persistence hooks. `useDailyGameplayPersistence.ts` is 433 lines and coordinates lock ownership, eligibility, restore readiness, saves, freezes, delivery recovery, completion creation and degradation using both refs and state. Neither exceeds the 500-line hard review threshold, but both are now close enough that further behavior should not accumulate there by default.

The earlier prerequisites are already in place: R2 extracted gameplay request lifetime, R1/R3 made owner authority explicit, and comparison reads live in separate identity-keyed client/controller/hooks and small presentation components. The remaining R7 work is therefore a targeted browser-orchestration decomposition, not a comparison redesign. Extract only cohesive responsibilities with concrete current consumers, keep gameplay rules in engine/Daily, keep comparison independent from persistence/write delivery, and do not introduce a generic repository/outbox/framework solely to reduce line count. No current correctness defect is asserted by R7; the goal is to make lifecycle ownership easier to reason about before the next feature adds pressure.
