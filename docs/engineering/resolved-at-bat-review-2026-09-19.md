...
At the September 20 checkpoint, `DailyInningGame.tsx` is 495 lines, combining view decisions, coordinated state setters, resolution transport, reset, hint hydration and composition of the comparison/persistence hooks. `useDailyGameplayPersistence.ts` was 433 lines and coordinated lock ownership, eligibility, restore readiness, saves, freezes, delivery recovery, completion creation and degradation using both refs and state. Neither exceeded the 500-line hard review threshold, but both were close enough that further behavior should not accumulate there by default.
...
R7B extracts the saved-game restore plus authorized-hint hydration lifecycle from `DailyInningGame` into a concrete web-only `dailySavedGameRestoreController`. The controller owns restore generations, bootstrap-bundle reuse, completed-save no-bundle handling, active-save hint hydration, and stale success/failure/settled fencing; the component still owns gameplay state, comparison invalidation, resolution-request invalidation, rendering and action dispatch. Reset and unmount explicitly invalidate both restore and resolution request lifetimes. This preserves current behavior while removing a second asynchronous lifecycle from the component without creating a generic state machine or changing persistence authority. After this checkpoint, review the remaining component concentration with fresh eyes before deciding whether a final R7C extraction is justified.
...
Review completion means evidence and next work are recorded. R1–R6 have bounded repairs; R7A and R7B are implemented as separate browser-architecture checkpoints, with a fresh-eye R7C decision checkpoint remaining before R7 is declared closed. R8 and the remaining interactive/mobile verification are still open.
