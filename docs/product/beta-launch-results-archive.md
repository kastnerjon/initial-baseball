...
The shared/engine layer defines the finalized pre-activation `points-v4` policy as HR 4, triple 3, double 2, single 1, walk 0.5, and strikeout/Give Up 0; wrong guesses one and two do not deduct points, while the third wrong guess remains a strikeout. Nine at-bats span 0 through 36 in 0.5-point steps. Portable schema-1 result contracts, exact numeric Supabase persistence, server result-write preflights, server-only comparison reads, and the isolated current-Daily v4 gameplay-save namespace support this exact-version contract, but this is still not a live switch: browser journal/outbox ownership, result production/delivery, scorecards/shares, comparison HTTP/browser acceptance, and the public default remain v3 until their separate rollout work. The earlier 4/3/2/1/0/-1 draft was redefined before any v4 production result existed, so no historical v4 population required reinterpretation. The owner also wants How to play on every game-page entry, terminal Jr/Sr omitted from generated initials, and supported stats shown in Baseball-Reference relative order with pitcher saves in hint 4. The implementation and clue-immutability consequences are scoped in `tasks/plans/2026-09-24-archive-and-gameplay-roadmap.md`.
...
Frozen permanent puzzles also have a provider-neutral read contract by permanent Daily number or permanent puzzle date within the versioned series. Reading an already-issued row does not require the application to know the launch epoch; that epoch is needed only to derive membership/identity for dates or numbers that are not already represented by a frozen record. The server-only Supabase adapter implements both lookups over the immutable table's existing unique keys and returns null for not-yet-issued rows. Server-only composition now wires that provider through the portable read service using one service-role client; future archive gameplay consumes this composition rather than querying Supabase directly. The read composition itself still returns frozen identity + canonical IDs only. A separate web materializer can now turn that immutable snapshot into the current `DailyPuzzle` gameplay shape while preserving the exact permanent batting order. It reuses the same canonical-ID gameplay-player lookup and pitch/hint adapter as current editorial Daily rather than inventing separate historical hint formatting, and it fails closed if a frozen canonical player cannot be materialized. A server-only archive puzzle source now composes the frozen reader and materializer for number/date lookup while preserving null for not-yet-issued puzzles. A separate server archive runtime reuses the current Daily runtime/progression-token machinery over that source; no scoring rule, token format/secret, or launch epoch is introduced. Not-yet-issued dates fail closed and signed claims remain bound to the permanent puzzle ID/date. Public archive routes remain separate, and archive browser persistence must be namespaced before those routes reuse the current gameplay UI.
