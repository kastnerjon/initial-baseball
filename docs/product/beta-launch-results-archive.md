...
The shared/engine layer now defines that September 24 proposal as `points-v4`: HR 4, triple 3, double 2, single 1, walk 0, and strikeout/Give Up -1; wrong guesses one and two do not deduct points, while the third wrong guess remains a strikeout. Negative totals are allowed and nine at-bats span -9 through 36. This is a versioned portable rule definition only, not yet live and not a reinterpretation of `points-v3` or historical `points-v2`; result transports, persistence, comparison, browser delivery, and the public default remain points-v3 until their separate rollout work. A later walk 0.5/strikeout 0 adjustment is permitted only as another explicit version, retaining old recorded scores and separate same-ruleset comparison populations. The owner also wants How to play on every game-page entry, terminal Jr/Sr omitted from generated initials, and supported stats shown in Baseball-Reference relative order with pitcher saves in hint 4. The implementation and clue-immutability consequences are scoped in `tasks/plans/2026-09-24-archive-and-gameplay-roadmap.md`.
