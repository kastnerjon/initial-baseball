...
The portable shared/engine layer now also defines `points-v4`: HR 4, 3B 3, 2B 2, 1B 1, BB 0, K/Give Up -1; wrong guesses one and two do not deduct, while the third wrong guess is K. Nine at-bats span -9 through 36 in integer steps. This definition is not a live switch: `CURRENT_DAILY_RULESET_VERSION`, browser gameplay, result validation/persistence, and comparison remain points-v3 until later versioned rollout work.
