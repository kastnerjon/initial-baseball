...
Hitter career/season reveal ordering is complete (`tasks/plans/hitter-reveal-column-order.md`). Pitcher-save data-contract prework now preserves known sourced zeroes while leaving an unavailable legacy career save total absent rather than fabricating `SV 0` (`tasks/plans/pitcher-saves-data-contract.md`). The next scoped concern remains hint-4 stat composition: consume structured supported stats, align their order with the reveal, and include `SV` only when supported.
