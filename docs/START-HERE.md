...
- Browser 6A and 6B are implemented but deliberately not activated. 6A provides the versioned fail-closed attempt journal plus immutable exact-payload resolved-AB outbox/retry client. 6B adds the separate browser concurrency adapter: one long-lived exclusive Web Lock per stable puzzle/`points-v3`, explicit owner/follower/unsupported state, abortable queued cleanup, durable reload before owner readiness, existing-journal generation fencing, and storage-event notification only. A missing journal remains missing in 6B; fresh/legacy eligibility and journal creation still belong to 6C. Gameplay does not yet gate writes on ownership, create journals, expose passive-follower UI, or call `POST /api/daily/at-bats`, so collection remains inactive. Scope: `tasks/plans/resolved-at-bat-browser-outbox.md`, `tasks/plans/resolved-at-bat-browser-ownership.md`, and `tasks/plans/resolved-at-bat-browser-lifecycle.md`.
...
1. Replacement roadmap #175, portable contract/validation #176, repository/service #178, Supabase provider #179, submission API #180, browser 6A journal/outbox, and browser 6B ownership coordinator are merged or implemented on the current workstream. Keep #174 draft.
2. Implement browser 6C: compose 6A + 6B with owner-gated gameplay persistence, fresh-run attempt creation, takeover rehydration, reset retirement, interrupted-write reconciliation and legacy/completed-result eligibility while collection remains off.
3. Implement browser 6D delivery activation, completion-ID reuse and multi-tab/mobile/production proof. Preserve old pending completed-result payloads exactly.
