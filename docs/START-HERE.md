...
The portable `points-v4` ruleset is defined in shared/engine: HR 4, 3B 3, 2B 2, 1B 1, BB 0, K/Give Up -1, with no deduction for wrong guesses one or two and an integer nine-AB range of -9..36. Portable schema-1 at-bat/completed-result contracts and engine validation accept both v3 and v4 while preserving exact-version scoring and idempotency identity. Portable Daily comparison math now also understands both versions: v3 keeps its 0–63 direct histogram, while v4 accepts signed AB sums and maps -9..36 into a 46-entry offset histogram; strict-lower beat math uses actual scores so negative values and ties behave correctly. `CURRENT_DAILY_RULESET_VERSION` remains `points-v3`, and the deployed comparison repository/Supabase provider/browser path remains v3-only. Next on the scoring track is row G3: widen Supabase result codecs/constraints plus comparison RPC/provider decoding for v4, with hosted privilege/RLS verification and no browser/default activation. Scopes: `tasks/plans/points-v4-engine-ruleset.md`, `tasks/plans/points-v4-result-contracts.md`, and `tasks/plans/points-v4-comparison-domain.md`.
