...
- Browser lifecycle planning is complete but not implemented. The approved design uses one exclusive Web Lock per puzzle/ruleset as cross-tab authority and a separate versioned local attempt journal/outbox. Only the owner writes the shared gameplay save or contribution state; takeover reloads durable state, and unsupported capability fails closed for AB collection. Implementation is split into four bounded PRs so persistence, concurrency, React lifecycle and network activation are reviewed independently. Scope: `tasks/plans/resolved-at-bat-browser-lifecycle.md`.
...
1. Replacement roadmap #175, portable contract/validation #176, repository/service #178, Supabase provider #179 and submission API #180 are merged. Keep #174 draft.
2. Implement browser 6A: versioned attempt journal plus immutable AB outbox client, with no React or live activation.
3. Implement browser 6B: exclusive Web Lock ownership/takeover coordinator; then 6C owner-gated gameplay/reset/legacy integration while collection remains off.
