...
- Browser 6A is implemented but deliberately not activated: the web layer now has a versioned fail-closed attempt journal plus immutable exact-payload resolved-AB outbox/retry client with active/retired contribution state, monotonic generation fencing data, terminal delivery classification, one-tab single-flight delivery and stale-response compare-before-write. Gameplay does not create journals or call `POST /api/daily/at-bats` yet. The approved exclusive Web Lock remains the next cross-tab authority; 6B must land before React/gameplay integration. Scope: `tasks/plans/resolved-at-bat-browser-outbox.md` and `tasks/plans/resolved-at-bat-browser-lifecycle.md`.
...
1. Replacement roadmap #175, portable contract/validation #176, repository/service #178, Supabase provider #179, submission API #180 and browser 6A journal/outbox are merged or implemented on the current workstream. Keep #174 draft.
2. Implement browser 6B: exclusive Web Lock ownership/takeover coordinator with generation fencing and unsupported-browser behavior. No gameplay mutation or network activation.
3. Implement browser 6C: owner-gated gameplay/reset/legacy integration while collection remains off; fresh eligibility and interrupted-write reconciliation belong here.
4. Implement browser 6D delivery activation, completion-ID reuse and multi-tab/mobile/production proof. Preserve old pending completed-result payloads exactly.
5. Implement and benchmark independent-population reads; then add asynchronous per-AB and final comparisons. Do not claim measured capacity or current hosting prices without checking.
6. Continue outstanding physical-device/editorial QA; archive/local-history follows the explicit future launch epoch. Remove public Reset before launch.
