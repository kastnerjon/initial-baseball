...
The portable `points-v4` ruleset is now defined in shared/engine only: HR 4, 3B 3, 2B 2, 1B 1, BB 0, K/Give Up -1, with no deduction for wrong guesses one or two and an integer nine-AB range of -9..36. `CURRENT_DAILY_RULESET_VERSION` remains `points-v3`; schema-1 result validation, persistence, comparison, browser delivery, archive activation, scorecards, and public gameplay have not been widened to v4. Next on the scoring track is roadmap row G: versioned result/comparison contracts and storage support, still without a public default switch. Scope: `tasks/plans/points-v4-engine-ruleset.md`.
