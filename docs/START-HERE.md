...
3. The portable independent-population comparison contract/service is implemented in `packages/daily`, and the server-only Supabase aggregate provider is implemented and hosted-verified on PR #198. PR #199 defines the portable versioned HTTP response contract in `packages/shared`. The current route PR composes that contract in `apps/web` through authoritative read-only GET endpoints: callers provide only date/ruleset plus pitch for an at-bat read, the server reloads the public puzzle and derives puzzle ID/number, and the existing Daily comparison service reads the hosted Supabase aggregates. The routes are deliberately dark by default and short-circuit before server/provider work unless `DAILY_NINE_COMPARISON_READS_ENABLED=true`; explicit activation waits for the representative performance gate. When enabled, responses are `no-store`, `cacheStatus: live`, and comparison failures are independent from gameplay/write acknowledgments. Browser/UI consumption remains later.
