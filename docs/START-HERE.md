...
September 24 owner decisions now add a coordinated archive/gameplay roadmap: isolate archive saves and session ownership first; reorder supported hitter reveal stats to match Baseball-Reference's relative order; align hint 4 and add supported pitcher saves; remove terminal Jr/Sr from generated initials; introduce a *new* Daily Nine scoring version (HR 4, 3B 3, 2B 2, 1B 1, BB 0, K/Give Up -1, no separate wrong-guess deduction); and open mode-appropriate How to play on every page entry. A future BB 0.5/K 0 rule remains reversible through another version, not historical rescore. At the start of this roadmap, permanent snapshots froze canonical IDs but not clues/hints; PRs #257–#261 now establish the v2 issuance contract before archive materialization consumes it. Full architecture, dependencies, PR scopes, rollout checks, and the initial no-code decision record: `tasks/plans/2026-09-24-archive-and-gameplay-roadmap.md`. Archive browser save/session isolation is complete in PR #253. Each newly started archive play uses the then-current public scoring version; earlier attempts keep their original version, and comparisons bind exact permanent puzzle plus played ruleset. See `tasks/plans/archive-browser-save-isolation.md`.

Hitter career/season reveal ordering is complete (`tasks/plans/hitter-reveal-column-order.md`). Pitcher-save data-contract prework preserves known sourced zeroes while leaving an unavailable legacy career save total absent rather than fabricating `SV 0` (`tasks/plans/pitcher-saves-data-contract.md`). Hint 4 now consumes structured career stats instead of the legacy `statsLine`, keeps its compact existing subset in reveal-relative order, and includes pitcher `SV` only when supported (`tasks/plans/hint-four-stat-composition.md`). Permanent issued-clue immutability is split into explicit layers: the portable clue snapshot contract freezes one four-slot public hint layout plus each ordered pitch's canonical player ID, public initials, and four materialized hint values without freezing scoring (`tasks/plans/permanent-daily-clue-snapshot-contract.md`). The schema-v2 envelope attaches that clue snapshot to the frozen batting order (`tasks/plans/permanent-daily-issued-puzzle-v2-contract.md`), and append-only Supabase persistence supports v1 + v2 with v1 backward reads and service-role SELECT/INSERT-only access (`tasks/plans/permanent-daily-issued-puzzle-v2-supabase.md`). PR #261 cuts server issuance over to v2 by freezing clues through the same canonical player lookup, `createDailyPuzzlePitch`, and `DEFAULT_DAILY_HINT_CONFIG` used by gameplay (`tasks/plans/permanent-daily-server-clue-issuance.md`). Archive materialization now consumes stored v2 labels, initials, hint order, and hint values while retaining schema-v1 reads; no scoring is frozen and no permanent row has been created. Scope: `tasks/plans/permanent-daily-archive-clue-materialization.md`. PR #263's initial day-boundary rollout failed review because it could change older beta initials when results were replayed. The owner approved keeping historical initials: dates before September 28, 2026 use the original Jr./Sr. initials; later dates omit them. The date-aware code is merged and production-verified ahead of that cutoff; the September 28 behavior has not yet become active. Names, aliases, interior tokens, and II/III remain unchanged. Scope: `tasks/plans/terminal-generational-suffix-initials.md`.

The portable `points-v4` ruleset, schema-1 result contracts, engine validation, immutable Supabase result storage, provider-neutral comparison math, and server-only Supabase comparison provider now all understand the signed 4/3/2/1/0/-1 policy while `CURRENT_DAILY_RULESET_VERSION` remains `points-v3`. **G3A** made exact-version v3/v4 result rows persistable; **G3B** made the comparison RPCs, signed decoder, and Daily repository/service read port exact-version v3/v4-capable. **H1** now widens only the server result-write preflight so `POST /api/daily/at-bats` and `POST /api/daily/results` can validate/store exact-version v4 submissions through those already-compatible lower layers. Browser journal/outbox and completed-result delivery, comparison HTTP/client/hooks, scorecards/shares, bootstrap selection and the public default intentionally remain v3-only, so no public scoring cutover occurs here. Scopes: `tasks/plans/points-v4-engine-ruleset.md`, `tasks/plans/points-v4-result-contracts.md`, `tasks/plans/points-v4-comparison-domain.md`, `tasks/plans/points-v4-result-supabase-persistence.md`, `tasks/plans/points-v4-comparison-supabase-provider.md`, and `tasks/plans/points-v4-result-write-api.md`.

## Permanent archive foundation

Archive PR 1 defines the portable `permanent-v1` series identity in `packages/daily` without choosing a launch date. Archive PR 2 adds the provider-neutral immutable issued-puzzle contract: stable `permanent-v1-daily-N` puzzle identity, exact ordered nine canonical player IDs, first issue timestamp, idempotent exact reissue, and conflict rejection for any rewrite. Append-only Supabase persistence implements that contract through `public.permanent_daily_issued_puzzles`, with server-only SELECT/INSERT access and no application update/delete path. Portable issuance orchestration accepts an explicitly resolved permanent identity plus the authoritative scheduled/published editorial puzzle for the same date, validates exact slots 1-9, and delegates freezing to the existing immutable service. The server-only web composition now binds that service to the authoritative editorial and permanent-issued-puzzle Supabase repositories through one service-role client, while still requiring the permanent identity explicitly and introducing no launch-epoch configuration. PR #261 cuts that issuance path over to schema v2 by freezing the exact current public initials, hint layout, labels, and values through the same canonical player lookup and `createDailyPuzzlePitch` adapter as gameplay. No public issuance or archive route exists yet. Current beta `DAILY_PUZZLE_EPOCH` numbering remains disposable. A provider-neutral read contract supports frozen permanent puzzle lookup by permanent series + Daily number or series + date without requiring a configured launch epoch. The existing server-only Supabase adapter now implements that read port against the table's unique number/date keys while reusing the same row decoder and query-error boundary; no schema, privilege, or write behavior changed. Server-only read composition now creates one service-role client, builds that provider, and delegates number/date access through the portable read service without launch-epoch inference. Archive materialization now supports both versions: v1 rebuilds clues as before; v2 uses the persisted hint layout, labels, initials, and values while resolving canonical player identity for reveal behavior. The portable read service preserves both versions. The server archive runtime reuses the existing Daily runtime and progression-token machinery with one archive-specific puzzle loader: `permanent-v1` + requested date must resolve to an issued frozen puzzle or gameplay fails closed. Existing token claims bind the permanent puzzle ID/date, retained rulesets remain caller-selected, and no token version/secret or scoring rule changed. Archive browser save/session identity is already isolated by PR #253; the archive UI remains unbuilt. Scopes: `tasks/plans/permanent-daily-identity.md`, `tasks/plans/permanent-daily-issued-puzzle.md`, `tasks/plans/permanent-daily-issued-puzzle-supabase.md`, `tasks/plans/permanent-daily-issuance.md`, `tasks/plans/permanent-daily-issuance-supabase-composition.md`, `tasks/plans/permanent-daily-issued-puzzle-read.md`, `tasks/plans/permanent-daily-issued-puzzle-supabase-read.md`, `tasks/plans/permanent-daily-issued-puzzle-read-composition.md`, `tasks/plans/permanent-daily-puzzle-materialization.md`, `tasks/plans/permanent-daily-archive-puzzle-source.md`, `tasks/plans/permanent-daily-archive-runtime.md`, and `tasks/plans/permanent-daily-server-clue-issuance.md`, plus `tasks/plans/permanent-daily-archive-clue-materialization.md`.
...
- The resolved-AB server API preflights only routing fields, rejects future Pacific dates, loads the authoritative cached public puzzle, calls exact-version engine validation/point derivation and stores through Daily/provider. H1 makes that server preflight accept `points-v3` and `points-v4`; `POST /api/daily/at-bats` still returns only created/existing/conflict/error status with `private, no-store`. The merged 6D owner path remains points-v3-only and invokes it asynchronously only after successful gameplay persistence and immutable journal freeze. Scopes: `tasks/plans/resolved-at-bat-submission-api.md` and `tasks/plans/points-v4-result-write-api.md`.
