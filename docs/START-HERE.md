...
The portable `points-v4` ruleset is defined in shared/engine: HR 4, 3B 3, 2B 2, 1B 1, BB 0, K/Give Up -1, with no deduction for wrong guesses one or two and an integer nine-AB range of -9..36. Portable schema-1 at-bat/completed-result contracts and engine validation now accept both v3 and v4 while preserving exact-version scoring and idempotency identity. `CURRENT_DAILY_RULESET_VERSION` remains `points-v3`; Supabase row codecs/constraints, comparison histograms/providers, browser delivery, archive activation, scorecards, and public gameplay have not been widened to v4. Next on the scoring track is row G2: make the portable comparison domain version-aware, including negative-score histogram offset semantics, before a separate Supabase migration/provider PR. Scopes: `tasks/plans/points-v4-engine-ruleset.md` and `tasks/plans/points-v4-result-contracts.md`.
