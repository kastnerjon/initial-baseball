...
2. Before comparison implementation, separately fix R1 outbox owner-lifetime fencing, R2 gameplay request/reset fencing, and R3 shared-save authority on journal failure. Address R4 re-bootstrap effect ordering before extending the persistence hook. Each fix needs focused lifecycle regression coverage and its own scope contract.
