...
Resolved-AB stack (browser activation code implemented; production proof pending)
...
- Browser 6A–6D implementation is complete in code, but resolved-AB collection is not yet declared production-proven. 6A owns the immutable attempt journal/outbox; 6B owns exclusive Web Lock authority/generation fencing; 6C owns points-v3 gameplay persistence and contribution eligibility; 6D adds the final save-ordered activation edge. Only after an owner gameplay save succeeds are persisted native terminal facts frozen into the journal; newly frozen slots are delivered asynchronously to `POST /api/daily/at-bats`, and pending slots retry after ownership/hydration without blocking gameplay. Conflict/rejection/stale/unavailable delivery fails the current contribution closed locally. A fresh active attempt ID is supplied only when creating a new points-v3 completed-result record; any existing completed-result record remains authoritative and retries unchanged. Classic/legacy/unsupported compatibility still sends no resolved ABs. Production declaration remains gated on exact merge-SHA deployment, multi-tab/browser proof, one controlled production insert + exact retry/readback, and a clean runtime-error scan. Scope: `tasks/plans/resolved-at-bat-browser-outbox.md`, `tasks/plans/resolved-at-bat-browser-ownership.md`, `tasks/plans/resolved-at-bat-browser-gameplay-lifecycle.md`, `tasks/plans/resolved-at-bat-browser-6d.md`, and `tasks/plans/resolved-at-bat-browser-lifecycle.md`. The last verified production baseline before 6D is 6C merge `51dfc16ae9d270903789301e154acc60dff45f49`, deployment `dpl_8L3rEB5aA1hV4nXrtddd1x1KM3nN`, READY with a clean post-deploy runtime-error scan.
...
PR #174 was inspected at `38f3bc9` and converted to draft. Do not merge unchanged: its every-AB-count-equals-completion-count rule is invalid for this product. Resolved-AB validation, immutable provider storage and the server POST boundary now exist, but browser collection and comparison reads are not active. Current browser identity starts at completion; progression tokens have no attempt ID/history. Cross-tab architecture and the reset/legacy policy are settled, but must be implemented and device-tested before activation. Full comparison scope: `tasks/plans/resolved-at-bat-comparison.md`; browser sequence: `tasks/plans/resolved-at-bat-browser-lifecycle.md`.
...
1. Replacement roadmap #175, portable contract/validation #176, repository/service #178, Supabase provider #179, submission API #180, and browser 6A–6D implementation are merged or implemented on the current workstream. Keep #174 draft.
2. Complete the 6D activation proof/reconciliation: exact merge-SHA production deploy, multi-tab takeover proof, controlled production AB insert + exact retry/readback, clean runtime-error scan, and canonical-doc reconciliation before declaring collection live.
