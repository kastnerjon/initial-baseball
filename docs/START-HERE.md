...
- September 17 production baseline is `ae2fc428b2fad05685b068944acce66b9dddf536` (PR #171), with READY production deployment `dpl_DctrheRJbonPmcszrjAfPuJxvzhy` on that exact merge SHA and no runtime errors in the first-hour scan. Completed-result 4A validation, 4B idempotency, the hardened Supabase provider, POST API, immutable browser delivery client, and native-completion activation are merged and live. Exact #171 head `8967b6d5145552613b2d3436e8395100848d45b3` passed CI run #620 and READY Vercel preview `dpl_9yGJDjnmyWYRLb2XjeAL4rLjkVZY`; production serves Daily #144 from the merged code. `public.daily_completed_results` still contained zero rows immediately after deployment, as expected before a fresh native browser completion. Result collection is not yet operationally proven: before 4D comparison work, complete one fresh Daily Nine or Classic session in a real browser, confirm exactly one row is persisted, and verify an identical same-ID retry does not increase row count. Draft PR #161 remains unmerged and must not be merged unchanged.
