...
- September 17 verified production code baseline is `9844931a2aff162324f7fa7e55190b6ee8d9e135` (PR #165), with READY production deployment `dpl_FzKSYzU3o1YZ1cQqA3t6ymnGd1H8` on that exact merge SHA. The private future-lineup workflow is operational through Daily #151. PRs #159 (4A) and #160 (4B) are merged. The completed-result Supabase provider is the current bounded concern: production already has an empty `daily_completed_results` table from applied migration `20260917132147`, and migration `20260918004822` hardens direct `service_role` access to `SELECT, INSERT` only. The table/adapter do not make submission live; the POST API and browser stable-ID/retry wiring remain separate follow-up PRs. Draft PR #161 remains unmerged and must not be merged unchanged.
...
- Completed-result step 4B implements the provider-neutral atomic repository/service boundary in `packages/daily`. `insertIfAbsent(result)` is the first-write-wins repository primitive keyed by `submissionId`; identical retries return the existing normalized result, while same-ID/different-payload retries return `idempotency_conflict` without overwrite. The service consumes 4A output and does not re-run validation/scoring. The current provider step adds the reconciled `daily_completed_results` migrations plus a server-only Supabase row codec/adapter that inserts first and reads the existing winner only after a PostgreSQL unique-key conflict. It has no update/upsert path and does not validate gameplay. Scope: `tasks/plans/completed-result-supabase-provider.md`. The completed-game API and browser submission/retry remain separate next work.
...
1. Finish/merge the bounded Supabase provider: reconcile the already-applied table migration, harden least-privilege grants, add the server-only codec/insert-first adapter, and verify advisors/CI/preview. Do not activate public submission in this PR.
2. Build a separate completed-result POST API that rejects malformed routing before puzzle construction, loads authoritative puzzle context, calls 4A validation/derivation, then stores through 4B/provider. Do not include browser hooks or comparison aggregates.
3. Build browser completion-only submission/retry as a separate PR with one stable persisted submission ID, same-ID retry across transient failure/refresh, native-fact provenance gating, and reset/in-flight race protection.
4. Continue remaining physical-device/editorial timed QA in parallel, then add same-Daily/same-ruleset comparison once collection is live and verified.
5. Build permanent archive/local-history infrastructure from the future explicit launch Daily #1; before broad launch choose the primary game/final rules/launch epoch and complete launch polish.
