...
2. R1 outbox owner-lifetime fencing and R2 gameplay request/reset fencing are implemented. Resolution requests now use synchronous single-flight request epochs; reset/restore/identity/access-loss invalidates old callbacks, and stale success/error/finally cannot overwrite a fresh run or clear a newer request. Before comparison implementation, fix R3 shared-save authority on journal failure and address R4 re-bootstrap effect ordering before extending the persistence hook.
