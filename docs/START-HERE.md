...
1. Browser 6A–6D implementation and final production browser proof are complete, including two-tab owner/follower/takeover, fresh completion-ID equality, physical Safari restore, exact-row cleanup, and clean route-error scans. Keep #174 draft.
2. Implement independent-population comparison contracts/provider/API, including read-only recovery, freshness semantics, and representative isolated performance measurements. Keep reads off the gameplay critical path.
3. Then add asynchronous YOU / AVG after each terminal AB plus final completed-game comparison using the already-settled separate populations and strict-lower tie semantics.
