...
- September 17 verified production code baseline is `0001f51c15b9e7b4e5e9647ce471365a96f19bc7` (PR #163), with READY production deployment `dpl_APaPW1hwmzghnoRg4fEXhcFnNCCw` on that exact merge SHA and a successful Vercel commit status. PRs #159 (4A) and #160 (4B) are merged; PR #162 broadened authorized manual selection without changing automatic generation, and PR #163 made public scheduled/published resolution use that same editorial candidate universe. Supabase already contains `daily_completed_results` with zero rows; its existence does not mean results submission is live. Draft PR #161 remains unmerged and must be split into bounded provider, API, and browser submission work before activation.
