...
The 4D foundation includes portable schema-1 points-v3 AB transport, pure engine validation/derived points, the Daily first-write-wins repository/service, a server-only Supabase provider, authoritative `POST /api/daily/at-bats` composition, and the full 6A–6D browser lifecycle. PR #174 remains draft; its equal-population invariant is superseded by replacement roadmap PR #175. The merged browser path now owns durable immutable observations, exclusive Web Lock authority, owner-gated gameplay persistence, save-ordered AB freeze/delivery, pending retry, and fresh attempt/completed-result identity reuse. Production endpoint/database behavior, physical multi-tab owner/follower/takeover, fresh full-completion identity reuse, and physical Safari restore are verified. The recorded production scenarios passed; the September 19 source review found ownership/request-lifetime counterexamples outside that proof. R1 owner-lifetime delivery fencing is now implemented: each delivery session is bound to one attempt/generation and is disposed before lock release, so late callbacks cannot acknowledge or continue under a successor owner. R2–R3 remain prerequisites before comparison implementation; review R4 also gates extension of the persistence hook. Findings and bounded PR sequence: `docs/engineering/resolved-at-bat-review-2026-09-19.md`. Scope: `tasks/plans/resolved-at-bat-contract.md`, `tasks/plans/resolved-at-bat-repository.md`, `tasks/plans/resolved-at-bat-supabase-provider.md`, `tasks/plans/resolved-at-bat-submission-api.md`, and `tasks/plans/resolved-at-bat-browser-lifecycle.md`.
...
2. R1 outbox owner-lifetime fencing is repaired with deterministic takeover and late-callback regressions. Before comparison implementation, separately fix R2 gameplay request/reset fencing and R3 shared-save authority on journal failure. Address R4 re-bootstrap effect ordering before extending the persistence hook. Each remaining fix needs focused lifecycle regression coverage and its own scope contract.
