...
2. R1 outbox owner-lifetime fencing is implemented: delivery is bound to a disposable exact attempt/generation session and cleanup invalidates it before lock release. R2 gameplay request fencing is also implemented: Reset and durable restore synchronously invalidate old Guess/Give Up callbacks, and stale success/error/finally work cannot affect a replacement request. Before comparison implementation, separately fix R3 shared-save authority on journal failure. Address R4 re-bootstrap effect ordering before extending the persistence hook. Each remaining fix needs focused lifecycle regression coverage and its own scope contract.
