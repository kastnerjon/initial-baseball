...
4. PR #206 adds the browser comparison transport/decoder and PR #207 adds the separate request-lifecycle controller with independent at-bat/completed channels, same-channel replacement, abort/invalidation and stale-response fencing. Next wire React ownership plus asynchronous YOU / AVG after terminal AB, then final completed-game comparison. Preserve independent populations and strict-lower ties, keep read recovery separate from write retries, and keep all comparison I/O off gameplay's critical path.
