...
3. Next implement the portable independent-population comparison contract/service as its own bounded concern, then provider aggregation and read-only API/recovery/freshness in separate PRs. Measure representative isolated mixed-load performance before UI activation.
