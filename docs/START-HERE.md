...
- Current runtime baseline is `4bb66d5bb93051dfb88559cfc58a524e06824f4a` (PR #212, R5 bounded resolved-AB delivery recovery). Push CI #782 completed successfully, including typecheck, tests, file-size checks, the canonical data pipeline and production package build. Production deployment `dpl_6rCfeRkUngD48ftT1FM8UgzjtCoZ` is `READY` on that exact SHA and targets production. PR #211 (`2fa9c61a8eb84d007bebdf2df89e40c70be230c2`) separately closed R6 malformed-save decoding. Neither R5 nor R6 changed Supabase. A later documentation-only reconciliation commit may advance GitHub `main` without changing this runtime baseline.
...
5. R5 bounded pending-delivery recovery and R6 malformed-save decoding are repaired. R7 remains deliberately split: R7A extracted mutable persistence owner-session mechanics behind one concrete web controller while keeping `useDailyGameplayPersistence` as the sole React effect/ordering authority; R7B extracts saved-game restore plus authorized-hint hydration/stale-response fencing from `DailyInningGame` into a separate concrete web controller. Reassess the remaining component concentration with fresh eyes before declaring R7 closed rather than forcing unrelated state-machine work into this checkpoint. R8 remains a separate server-boundary concern. Continue physical-device/editorial QA. Archive/local history follows the explicit future launch epoch. Remove public Reset before launch.
