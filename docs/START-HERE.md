...
Hitter career/season reveal ordering is complete (`tasks/plans/hitter-reveal-column-order.md`). Pitcher-save data-contract prework preserves known sourced zeroes while leaving an unavailable legacy career save total absent rather than fabricating `SV 0` (`tasks/plans/pitcher-saves-data-contract.md`). Hint 4 now consumes structured career stats instead of the legacy `statsLine`, keeps its compact existing subset in reveal-relative order, and includes pitcher `SV` only when supported (`tasks/plans/hint-four-stat-composition.md`). The next scoped concern is permanent issued-clue immutability before changing Jr/Sr initials or issuing permanent Daily #1.
