...
The portable `points-v4` ruleset, schema-1 result contracts, engine validation, and provider-neutral comparison math all understand the signed 4/3/2/1/0/-1 policy while `CURRENT_DAILY_RULESET_VERSION` remains `points-v3`. G3 is now explicitly decomposed: **G3A** widens only the immutable Supabase result codecs/table constraints so v3 and v4 rows can coexist under exact-version bounds without changing any browser/API/default path; **G3B** remains the next concern and will widen the comparison RPC/provider boundary for signed v4 aggregates. The deployed comparison repository/RPC/browser path therefore remains points-v3-only after G3A. Scopes: `tasks/plans/points-v4-engine-ruleset.md`, `tasks/plans/points-v4-result-contracts.md`, `tasks/plans/points-v4-comparison-domain.md`, and `tasks/plans/points-v4-result-supabase-persistence.md`.
