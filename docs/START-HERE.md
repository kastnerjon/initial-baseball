...
3. The portable independent-population comparison contract/service is implemented in `packages/daily`, and the server-only Supabase aggregate provider is implemented and hosted-verified on PR #198: exact per-slot count/point-sum reads remain separate from completed-game score buckets, raw population rows do not leave Postgres, and the read functions are SECURITY INVOKER with service-role-only execution. Hosted migration `20260919164818_create_daily_nine_comparison_reads` is applied; ACL/readback and service-role behavior passed. The next bounded concern is the read-only API/recovery/freshness contract, then representative isolated mixed-load performance before UI activation.
