...
- September 17 current code baseline is `e0d3eb2719fbd14b4b77f967133d501b37686a98` (PR #168) on `main`. PR #170 also merged the underlying Turbo/baseball-data build-race fix before #168. The private future-lineup workflow is operational through Daily #151. Completed-result 4A validation, 4B idempotency, the hardened Supabase provider, the completed-game POST API, and the dormant browser delivery client are merged. Production `daily_completed_results` remained at zero rows immediately after #168 merged because gameplay activation had not landed. The current bounded concern is native-completion activation: browser-local save provenance plus React/game composition only. Draft PR #161 remains unmerged and must not be merged unchanged.
...
1. Finish/merge the native-completion activation PR: hydration-only provenance plus React/game composition only; no client/API/schema changes.
2. Verify exact production deployment, then verify live collection with one genuine native completion/readback and an identical same-ID retry that creates no second row before treating analytics collection as fully operational.
