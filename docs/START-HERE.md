...
The portable `points-v4` ruleset is finalized pre-activation as 4/3/2/1/0.5/0, with no nonterminal wrong-guess deduction, a 0..36 nine-AB range, and 0.5-point score steps. Schema-1 result contracts, engine validation, exact numeric Supabase result storage, provider-neutral comparison math, the server-only comparison provider, and **H1 server result-write preflights** now support that exact-version contract while `CURRENT_DAILY_RULESET_VERSION` remains `points-v3`. H1 means `POST /api/daily/at-bats` accepts v3/v4 and `POST /api/daily/results` accepts v3/v4/Classic, with the exact routed ruleset passed to authoritative puzzle loading and engine validation; it does **not** activate a v4 browser producer. The browser journal/outbox/ownership path, completed-result browser creation, shared comparison HTTP/read-service/browser consumers, current-Daily save compatibility, How-to copy, scorecards/shares and the public default remain later H work. In particular, current non-archive save compatibility must become exact-version before the default switch so an in-progress v3 save can never hydrate as v4. Scopes: `tasks/plans/points-v4-engine-ruleset.md`, `tasks/plans/points-v4-result-contracts.md`, `tasks/plans/points-v4-comparison-domain.md`, `tasks/plans/points-v4-result-supabase-persistence.md`, `tasks/plans/points-v4-comparison-supabase-provider.md`, `tasks/plans/points-v4-half-walk-zero-strikeout.md`, and `tasks/plans/points-v4-result-write-api.md`.
