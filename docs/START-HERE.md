...
3. The portable independent-population comparison contract/service is implemented in `packages/daily`, and the server-only Supabase aggregate provider is implemented on the current provider PR: exact per-slot count/point-sum reads remain separate from completed-game score buckets, raw population rows do not leave Postgres, and the read functions are SECURITY INVOKER with service-role-only execution. Hosted migration/readback is the remaining verification gate on this PR. After this PR, implement the read-only API/recovery/freshness contract as a separate concern, then measure representative isolated mixed-load performance before UI activation.
