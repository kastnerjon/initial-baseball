...
3. The portable independent-population comparison contract/service is implemented in `packages/daily`, and the server-only Supabase aggregate provider is implemented and hosted-verified on PR #198. PR #199 defines the portable versioned HTTP response contract in `packages/shared`. PR #200 owns only server-side comparison read composition in `apps/web`: it validates date/ruleset/pitch routing inputs, reloads the authoritative public puzzle, derives puzzle ID/number server-side, composes the existing Daily comparison service with the hosted Supabase provider, and captures conservative live `sourceReadAt`. No public route is added in this PR. The next bounded PR adds the thin Next.js GET adapters, sanitized HTTP mapping, and a default-off activation gate; performance/cache and browser/UI work remain later.
