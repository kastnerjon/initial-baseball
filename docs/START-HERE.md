...
The portable `points-v4` ruleset is now finalized pre-activation as 4/3/2/1/0.5/0, with no nonterminal wrong-guess deduction, a 0..36 nine-AB range, and 0.5-point score steps. Schema-1 result contracts, engine validation, immutable Supabase result storage, provider-neutral comparison math, and the server-only Supabase comparison provider all support that exact-version contract while `CURRENT_DAILY_RULESET_VERSION` remains `points-v3`. **G3A/G3B** originally established the v4 persistence/read seams; the September 26 redefinition widened the numeric storage and aggregate contracts before any v4 production result existed. The shared comparison HTTP schema, request service, browser client/hooks, scorecards and public default intentionally remain points-v3-only until H, so no public scoring cutover occurs here. Scopes: `tasks/plans/points-v4-engine-ruleset.md`, `tasks/plans/points-v4-result-contracts.md`, `tasks/plans/points-v4-comparison-domain.md`, `tasks/plans/points-v4-result-supabase-persistence.md`, and `tasks/plans/points-v4-comparison-supabase-provider.md`.
