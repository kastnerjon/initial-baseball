...
3. The portable independent-population comparison contract/service is implemented in `packages/daily`, and the server-only Supabase aggregate provider is implemented and hosted-verified on PR #198. The current bounded PR defines the portable versioned HTTP response contract in `packages/shared`: exact comparison identity, separate at-bat/completed payloads, explicit freshness (`sourceReadAt`, `servedAt`, `cacheStatus`) and sanitized comparison-read error codes. The next PR owns only the Next.js/server read routes and authoritative request composition; performance/cache decisions remain later.
