...
The portable `points-v4` ruleset, schema-1 result contracts, engine validation, immutable Supabase result storage, provider-neutral comparison math, and server-only Supabase comparison provider now all understand the signed 4/3/2/1/0/-1 policy while `CURRENT_DAILY_RULESET_VERSION` remains `points-v3`. **G3A** made exact-version v3/v4 result rows persistable; **G3B** makes the comparison RPCs, signed decoder, and Daily repository/service read port exact-version v3/v4-capable under the same service-role-only access posture. The shared comparison HTTP schema, request service, browser client/hooks, scorecards and public default intentionally remain points-v3-only until H, so no public scoring cutover occurs here. Scopes: `tasks/plans/points-v4-engine-ruleset.md`, `tasks/plans/points-v4-result-contracts.md`, `tasks/plans/points-v4-comparison-domain.md`, `tasks/plans/points-v4-result-supabase-persistence.md`, and `tasks/plans/points-v4-comparison-supabase-provider.md`.
