...
4. PR #206 adds the browser comparison transport/decoder and PR #207 adds the separate request-lifecycle controller. Terminal Daily Nine at-bats now consume those seams through a dedicated hook/presentation path: own results remain immediate, comparison is asynchronous, sample/outage states are explicit, and advance/reset/restore fence obsolete reads. Next add completed-game comparison and the final scorecard/strict-lower finisher view, then measure real end-to-end comparison latency. Preserve independent populations, keep read recovery separate from write retries, and keep all comparison I/O off gameplay's critical path.
