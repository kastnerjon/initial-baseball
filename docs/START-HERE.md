...
3. The portable independent-population comparison contract/service is implemented in `packages/daily`: per-slot count/point-sum reads remain separate from completed-game score-bucket reads, empty averages are null, strict-lower ties are explicit, with freshness/caching intentionally deferred to the later web read contract. Next implement the Supabase aggregate provider, then read-only API/recovery, each as separate PRs. Measure representative isolated mixed-load performance before UI activation.
