...
1. Build the completed-result POST API that rejects malformed routing before puzzle construction, loads authoritative puzzle context, calls 4A validation/derivation, then stores through 4B/Supabase provider. Do not include browser hooks or comparison aggregates.
2. Build browser completion-only submission/retry as a separate PR with one stable persisted submission ID, same-ID retry across transient failure/refresh, native-fact provenance gating, and reset/in-flight race protection.
3. Verify live result collection end to end without per-action writes or answer leakage, while continuing remaining physical-device/editorial timed QA in parallel.
