...
- Browser 6A–6C are implemented while resolved-AB delivery remains deliberately inactive. 6A owns the versioned attempt journal/outbox; 6B owns exclusive Web Lock authority and generation fencing; 6C composes them with points-v3 gameplay persistence. A genuinely fresh owned run requires the gameplay key to be truly absent and creates its journal before the first gameplay save; an unreadable/corrupt persisted value is not treated as fresh. Pre-rollout saves without a journal remain completion-only, takeover reloads durable state before interaction, mismatched gameplay/journal facts retire contribution without backfill, reset retires observed attempts before clearing gameplay, failed gameplay writes fail contribution closed, and supported followers are passive/non-writing. Classic, older rulesets and unsupported browsers retain existing compatibility gameplay rather than gaining AB contribution. New completed-result creation is disabled for followers and retired/mismatched replays, while existing pending completed-result records remain independently retryable. No gameplay path calls `POST /api/daily/at-bats` yet. Scope: `tasks/plans/resolved-at-bat-browser-outbox.md`, `tasks/plans/resolved-at-bat-browser-ownership.md`, `tasks/plans/resolved-at-bat-browser-gameplay-lifecycle.md`, and `tasks/plans/resolved-at-bat-browser-lifecycle.md`. The 6B production baseline is verified READY on `2f3204d9de208ee8d15426404802b1c9b14faeb8` as `dpl_E3NBZuDcZvYKyZicgDcm5L3mexLZ`; the post-deploy runtime-error scan was clean.
...
1. Replacement roadmap #175, portable contract/validation #176, repository/service #178, Supabase provider #179, submission API #180, and browser 6A–6C are merged or implemented on the current workstream. Keep #174 draft.
2. Implement browser 6D: after a successful owner gameplay save, freeze/send/retry the exact native terminal AB, reuse the fresh attempt ID for a new completed-result record when eligible, and prove multi-tab/mobile/production behavior before declaring collection live. Preserve existing completed-result records exactly.
