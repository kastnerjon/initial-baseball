...
5. R5 bounded pending-delivery recovery and R6 malformed-save decoding are repaired. R7 is deliberately split: the first checkpoint extracts mutable persistence owner-session mechanics behind one concrete web controller while keeping `useDailyGameplayPersistence` as the sole React effect/ordering authority; `DailyInningGame` orchestration remains a separate R7 follow-up. R8 remains a separate server-boundary concern. Continue physical-device/editorial QA. Archive/local history follows the explicit future launch epoch. Remove public Reset before launch.
