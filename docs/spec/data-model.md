...
The approved browser lifecycle adds no server table. Browser 6A implements the version-1 local attempt journal keyed by stable puzzle ID/date plus exact `points-v3`; browser 6B adds ephemeral exclusive-lock ownership and generation fencing; browser 6C now decides when that journal may participate in gameplay. Fresh owned points-v3 runs create the journal before the first gameplay save only when the gameplay storage key is truly absent; an unreadable/corrupt persisted value is not fresh. Existing points-v3 saves without a journal remain completion-only. Existing active journals survive takeover only when their generation matches the ownership claim and every immutable observation exactly matches the persisted native terminal facts; any reconstructed, missing, ahead or inconsistent fact set retires contribution without copying gameplay facts into the journal. Reset with zero observations may retain the attempt, while reset after any observation retires the journal before gameplay clear. Failed gameplay persistence retires the current contributing session locally. Supported followers have no gameplay-write authority and cannot create new completed-result records; existing completed-result retry records remain authoritative and independent. Classic, older rulesets and unsupported browsers keep compatibility gameplay but do not gain AB contribution. No terminal AB is appended or posted by 6C, so `POST /api/daily/at-bats` remains inactive from gameplay. 6D will add gameplay-save → immutable observation append → asynchronous POST and fresh attempt-ID reuse for new completed-result records. Scope: `tasks/plans/resolved-at-bat-browser-outbox.md`, `tasks/plans/resolved-at-bat-browser-ownership.md`, `tasks/plans/resolved-at-bat-browser-gameplay-lifecycle.md`, and `tasks/plans/resolved-at-bat-browser-lifecycle.md`.
