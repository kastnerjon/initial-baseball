...
AB comparisons aggregate these independent observations, including partial games, using `resolvedAtBatCount`. The portable `packages/daily` comparison read contract asks a provider for exactly one slot's `resolvedAtBatCount` plus `awardedPointsSum`; it derives `averagePoints`, returning null when the count is zero. Whole-game comparison is a separate repository method over `daily_completed_results`: providers return bounded score/count buckets, and Daily derives `completedGameCount`, null-or-average points, a 64-entry 0–63 histogram, and strict-lower finishers rate. Ties remain in the denominator and are not counted as beaten. Freshness/cache metadata is intentionally not part of the portable Daily comparison data contract; the later web/API read layer owns it. The Supabase provider uses two server-only aggregate RPCs installed by `20260919164818_create_daily_nine_comparison_reads`: one returns exact resolved-AB count plus stored awarded-point sum, and one returns completed-game score/count buckets. Both are SECURITY INVOKER and executable only by service_role; hosted ACL/readback and service-role execution are verified, and no raw anonymous population rows are returned to Node. No equality or monotonic-slot-count invariant is valid across AB slots or completed games. SQL may aggregate persisted engine-derived points but must not implement scoring. Do not backfill completed records into AB observations. Classic remains a separate population with baseball-native measures.
