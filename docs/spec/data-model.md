...
The approved browser lifecycle adds no server table. A separate versioned local attempt journal will hold stable puzzle/ruleset identity, one attempt ID, active/retired contribution state, an ownership generation and at most nine immutable schema-1 observation payloads with delivery status. It is browser delivery metadata, not portable gameplay or aggregate authority. The journal is coordinated by an exclusive Web Lock; `localStorage` alone is not treated as a transaction. Terminal AB persistence orders gameplay save before journal append before POST; reset after an observation retires the journal before clearing gameplay. A hydration mismatch retires contribution rather than backfilling. Old saves without the journal remain completion-only, and existing completed-result records remain authoritative. Scope: `tasks/plans/resolved-at-bat-browser-lifecycle.md`.
