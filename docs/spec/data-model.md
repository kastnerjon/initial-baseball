...
Hosted verification used two simultaneous disposable same-key inserts with differing normalized payloads. Exactly one row won; cleanup returned the table to zero rows. This proves the database uniqueness primitive, not coherent browser attempt ownership. `POST /api/daily/at-bats` now composes authoritative puzzle lookup, engine normalization/point derivation, Daily idempotency and this provider without adding browser collection. Scope: `tasks/plans/resolved-at-bat-supabase-provider.md` and `tasks/plans/resolved-at-bat-submission-api.md`.

The approved browser lifecycle adds no server table. Browser 6A implements a version-1 local attempt journal keyed by stable puzzle ID/date plus exact `points-v3`. It stores stable puzzle number/ruleset identity, one schema-valid attempt ID, active/retired contribution state, a monotonic ownership generation, and at most nine immutable schema-1 `DailyAtBatResultSubmission` payloads with `pending`/`submitted`/`conflict`/`rejected` delivery status. The codec fails closed on corrupt/mismatched state; same-slot fact disagreement never overwrites the frozen payload and instead retires future contribution. Network/408/425/429/5xx leaves the exact stored payload pending; 409 or ordinary invalid 4xx is terminal and retires future contribution; terminal async responses may update only the matching generation and exact stored payload. Pending observations remain retryable even after retirement. Browser 6B adds no new persisted record: under one exclusive Web Lock it advances and verifies the generation of an existing journal before owner readiness, while a missing journal remains missing so 6C alone can decide whether a fresh eligible run may create one. Owner/follower/unsupported concurrency state is ephemeral; storage events are notification only. This remains browser delivery/concurrency metadata, not portable gameplay or aggregate authority. No gameplay path creates this journal or calls the resolved-AB POST yet, so collection remains inactive. Later terminal persistence orders gameplay save before journal append before POST; reset after an observation retires the journal before clearing gameplay. A hydration mismatch retires contribution rather than backfilling. Old saves without the journal remain completion-only, and existing completed-result records remain authoritative. Scope: `tasks/plans/resolved-at-bat-browser-outbox.md`, `tasks/plans/resolved-at-bat-browser-ownership.md`, and `tasks/plans/resolved-at-bat-browser-lifecycle.md`.
