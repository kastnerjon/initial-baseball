...
- G3B migration `20260925193811_widen_points_v4_comparison_reads` widens only the existing service-role comparison functions from v3 to the supported Daily Nine set `points-v3`/`points-v4`, retaining SECURITY INVOKER and explicit function grants;
...
The table/adapter is consumed only through the server completed-game POST boundary. H1 makes that server boundary accept exact-version `points-v3`, `points-v4`, and Classic submissions, while the ordinary browser submission/retry client remains on the current v3/Classic activation path. The browser client is separate from gameplay persistence and creates a completed-result record only from an eligible genuine completion, reusing an active fresh points-v3 attempt ID when 6D contribution is active.
...
AB comparisons aggregate these independent observations, including partial games, using `resolvedAtBatCount`. Portable Daily comparison normalization is now exact-version for `points-v3` and `points-v4`: it validates AB count/sum pairs against the engine-owned per-AB range, derives null-or-average points, converts completed score/count buckets into a ruleset-offset histogram, and owns strict-lower finishers semantics. V3 therefore remains a 64-entry 0–63 histogram; v4 is a 46-entry histogram where index 0 represents -9 and index 45 represents 36. Ties remain in the denominator and are not counted as beaten, including negative scores. G3B widens the comparison repository/service read port and the two server-only aggregate RPCs to exact-version points-v3/points-v4 populations. The RPCs remain SECURITY INVOKER and executable only by service_role; migration `20260925193811_widen_points_v4_comparison_reads` adds no public function access and no raw anonymous population rows are returned to Node. The provider decoder accepts signed safe integer sums/scores, while Daily still validates those values against the engine-owned exact-version ranges. The shared HTTP/browser comparison contract remains points-v3-only until H. Freshness/cache metadata remains web/API-owned. No equality or monotonic-slot-count invariant is valid across AB slots or completed games. SQL may aggregate persisted engine-derived points but must not implement scoring. Do not backfill completed records into AB observations. Classic remains a separate population with baseball-native measures.
