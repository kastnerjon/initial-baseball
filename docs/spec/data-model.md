...
The relational `daily_completed_results` table plus server-only Supabase row codec/repository adapter are implemented as the provider portion of 4C. The completed-game POST API validates against the authoritative public puzzle through engine 4A and stores through 4B/provider. The browser submission client now stores a separate local marker keyed by stable puzzle identity plus exact ruleset/game. It persists the submission ID before network I/O, reuses it for transient retry, and records terminal submitted/conflict/rejected status. The client is not yet wired to game completion; ordinary gameplay still does not call the endpoint until the native-provenance activation step lands. The browser already records the native raw facts needed to form a future submission, but is not yet wired to this contract. Legacy facts reconstructed from old local pitch lines are compatibility display data and must not be submitted without an explicit migration rule. There are no per-action writes.
