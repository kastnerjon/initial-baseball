...
- G3A migration `20260925184454_widen_points_v4_result_persistence` first widened exact-version v4 storage under the original inactive integer/signed draft; migration `20260926220726_redefine_points_v4_fractional_scoring` supersedes those v4 bounds before activation by storing at-bat points as exact `numeric(4,1)`, enforcing v3 integer `0..7` and v4 half-step `0..4`, and enforcing completed v4 totals `0..36` in 0.5-point steps;
- G3B migration `20260925193811_widen_points_v4_comparison_reads` first widened the existing service-role comparison functions to v3/v4; migration `20260926220726_redefine_points_v4_fractional_scoring` changes only their point-valued return columns to exact numeric values so v4 half-points survive aggregation, retaining SECURITY INVOKER and explicit function grants;
...
The table/adapter is consumed only through the server completed-game POST boundary. The browser submission/retry client is separate from gameplay persistence and creates a completed-result record only from an eligible genuine completion, reusing an active fresh points-v3 attempt ID when 6D contribution is active.
...
New games still use `points-v3`: each at-bat starts at 7 points, each revealed hint or wrong guess deducts 1, and a third wrong guess or Give Up awards 0; nine at-bats have a 63-point maximum. Shared/engine define `points-v4` as the future `4/3/2/1/0.5/0` outcome policy with no deduction for the first two wrong guesses and a `0..36` nine-at-bat range in 0.5-point steps. Immutable result storage, the server result-write preflights, the server comparison provider/read port, and H2 browser gameplay-save isolation are compatible with exact-version v4 half-point identity. H2 preserves the historical pre-v4 `initial-baseball:daily:<date>` key for legacy/v1/v2/v3 compatibility but stores v4 under `initial-baseball:daily:ruleset:points-v4:<date>`, so a v4 play cannot consume or overwrite a v3 save. Browser journal/outbox ownership, result creation-delivery, shared comparison HTTP/browser acceptance and the public default remain points-v3 until later H work. Compatible `points-v2` saves retain `4/3/2/1/0.5/0` and a 36-point maximum; `points-v1` saves and signed tokens retain `5/4/3/2/1/0` and a 45-point maximum. Compatible pre-ruleset saves and signed tokens normalize to `legacy-inning-v1` so an already-started game is not silently changed from three-out completion to all-scheduled-at-bats completion.
...
Exact transport fields are `schemaVersion`, `submissionId`, `puzzleId`, `puzzleDate`, `puzzleNumber`, `rulesetVersion`, and `completedAtBats`. Portable schema-1 validation accepts `points-v3`, `points-v4`, and `classic-inning-v1`. The ruleset identifies the game independently of the puzzle ID; neither game requires the other game or its lineup to exist. Server-only Supabase storage, server result-write preflights and comparison-provider reads support exact-version v4, including the finalized half-point contract. The completed-result server boundary accepts v3/v4/Classic and the resolved-AB boundary accepts v3/v4; ordinary browser submission creation/delivery and public comparison HTTP/browser acceptance remain points-v3/Classic-only until later H work.
...
AB comparisons aggregate these independent observations, including partial games, using `resolvedAtBatCount`. Portable Daily comparison normalization is exact-version for `points-v3` and `points-v4`: it validates AB count/sum pairs against the engine-owned per-AB range and score step, derives null-or-average points, converts completed score/count buckets into a ruleset-step histogram, and owns strict-lower finishers semantics. V3 remains a 64-entry 0–63 histogram; v4 is a 73-entry histogram where index 0 represents 0, index 1 represents 0.5, and index 72 represents 36. Ties remain in the denominator and are not counted as beaten. The comparison repository/service read port and two server-only aggregate RPCs support exact-version points-v3/points-v4 populations. The RPCs remain SECURITY INVOKER and executable only by service_role; the September 26 migration changes their point-valued returns to exact numeric values without adding public function access or returning raw anonymous population rows to Node. The provider decoder accepts safe half-point sums/scores while counts remain safe integers, and Daily still validates values against the engine-owned exact-version ranges and steps. The shared HTTP/browser comparison contract remains points-v3-only until H. Freshness/cache metadata remains web/API-owned. No equality or monotonic-slot-count invariant is valid across AB slots or completed games. SQL may aggregate persisted engine-derived points but must not implement scoring. Do not backfill completed records into AB observations. Classic remains a separate population with baseball-native measures.

New contributing runs establish anonymous identity before the first AB and should reuse it for completion without changing existing pending schema-1 payloads. Old saves without new provenance remain completion-only under existing eligibility; no synthetic historical ABs. Delivery acknowledgments are independent of optional comparison reads. Comparison contract: `tasks/plans/resolved-at-bat-comparison.md`; full identity/reset/multiple-tab architecture: `tasks/plans/resolved-at-bat-browser-lifecycle.md`.
...
Hosted verification used two simultaneous disposable same-key inserts with differing normalized payloads. Exactly one row won; cleanup returned the table to zero rows. This proves the database uniqueness primitive, not coherent browser attempt ownership. `POST /api/daily/at-bats` composes authoritative puzzle lookup, engine normalization/point derivation, Daily idempotency and this provider; browser 6D now invokes it asynchronously after successful owner gameplay persistence and immutable journal freeze. Scope: `tasks/plans/resolved-at-bat-supabase-provider.md` and `tasks/plans/resolved-at-bat-submission-api.md`.

The approved browser lifecycle adds no server table. Browser 6A defines the version-1 local attempt journal/outbox; 6B defines ephemeral exclusive-lock authority and generation fencing; 6C defines gameplay contribution eligibility; 6D activates delivery without changing that persisted schema. A successful owner gameplay save is the prerequisite for freezing native terminal facts into immutable schema-1 AB submissions. Newly frozen slots may then POST asynchronously, and pending exact payloads retry after ownership/hydration. Conflict/rejection retires the journal under 6A semantics; stale/unavailable delivery also disables the current local contribution session. The browser never backfills observations from saved gameplay after an interrupted save-before-freeze edge. For new points-v3 completed-result records only, the active journal `attemptId` is supplied as the completed-result `submissionId`; if a completed-result browser record already exists, its frozen ID/payload/status remains authoritative and ignores the preferred attempt ID. Legacy/completion-only and Classic creation keep independent IDs. Existing completed-result retry records remain unchanged. Classic, older rulesets and unsupported browsers send no resolved ABs. Production collection is verified by the separate 6D deployment/browser/database proof: current-browser owner/follower/takeover, one nine-AB run with attempt/completion ID equality, physical Safari restore, exact cleanup and clean route runtime scans. Scope: `tasks/plans/resolved-at-bat-browser-outbox.md`, `tasks/plans/resolved-at-bat-browser-ownership.md`, `tasks/plans/resolved-at-bat-browser-gameplay-lifecycle.md`, `tasks/plans/resolved-at-bat-browser-6d.md`, and `tasks/plans/resolved-at-bat-browser-lifecycle.md`.