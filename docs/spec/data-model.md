...
The approved browser lifecycle adds no server table. Browser 6A implements a version-1 local attempt journal keyed by stable puzzle ID/date plus exact `points-v3`. It stores stable puzzle number/ruleset identity, one schema-valid attempt ID, active/retired contribution state, a monotonic ownership generation, and at most nine immutable schema-1 `DailyAtBatResultSubmission` payloads with `pending`/`submitted`/`conflict`/`rejected` delivery status. The codec fails closed on corrupt/mismatched state; same-slot fact disagreement never overwrites the frozen payload and instead retires future contribution. Network/408/425/429/5xx leaves the exact stored payload pending; 409 or ordinary invalid 4xx is terminal and retires future contribution; terminal async responses may update only the matching generation and exact stored payload. Pending observations remain retryable even after retirement. This is browser delivery metadata, not portable gameplay or aggregate authority. No gameplay path creates this journal or calls the resolved-AB POST yet, so collection remains inactive. The journal will be coordinated by the 6B exclusive Web Lock; `localStorage` alone is not treated as a transaction. Later terminal persistence orders gameplay save before journal append before POST; reset after an observation retires the journal before clearing gameplay. A hydration mismatch retires contribution rather than backfilling. Old saves without the journal remain completion-only, and existing completed-result records remain authoritative. Scope: `tasks/plans/resolved-at-bat-browser-outbox.md` and `tasks/plans/resolved-at-bat-browser-lifecycle.md`.
