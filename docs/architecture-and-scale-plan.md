...
2. Follow `tasks/plans/resolved-at-bat-comparison.md`: resolved-AB contracts and provider are complete; next add the submission API, then browser attempt coordination, measured independent-population reads, and asynchronous reveal/final UI. Supabase is the provider; engine owns scoring; React renders.
