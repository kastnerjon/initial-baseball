...
`shared` exports schema-1 submission/result types. Engine `validateDailyCompletedResult` now accepts `points-v3`, `points-v4`, and `classic-inning-v1`, checks the caller-provided puzzle/game and raw facts, and reuses the exact-version engine rules for derivation/completion. `validateDailyAtBatResult` likewise accepts only the two supported Daily Nine result versions, derives awarded points from the exact version, and preserves exact ruleset identity. Output keeps copied, whitelisted facts and drops client totals/answer fields. G3A widens the Supabase result codecs/constraints to match those portable v4 contracts; G3B widens the server-only comparison RPC/provider/read port to signed exact-version v3/v4 populations; H1 widens the two server result-write preflights to exact-version v3/v4 without changing browser activation. Browser journal/outbox and completed-result delivery, shared comparison HTTP acceptance, scorecard/share presentation and the current public default remain H work. See `docs/spec/engine.md`, `docs/spec/data-model.md`, `tasks/plans/points-v4-result-contracts.md`, `tasks/plans/points-v4-comparison-supabase-provider.md`, and `tasks/plans/points-v4-result-write-api.md`.
...
The Daily comparison math has a provider-neutral exact-version seam for points-v3 and points-v4. It obtains score ranges from engine rules, accepts signed v4 AB sums, uses score-minus-minimum offset histograms for completed distributions, and calculates strict-lower rates over actual scores rather than array indices. G3B widens the repository/service read port and Supabase RPC decoder together: SQL still only aggregates persisted engine-derived values, provider decoding accepts signed safe integers, and Daily validates exact-version score ranges. The shared HTTP/browser comparison contract remains points-v3-only until H. Scopes: `tasks/plans/points-v4-comparison-domain.md` and `tasks/plans/points-v4-comparison-supabase-provider.md`.
...
`POST /api/daily/at-bats` is the separate resolved-AB submission boundary. It preflights schema/date/exact `points-v3` or `points-v4` routing, rejects future Pacific dates, loads the authoritative cached public puzzle without minting progression tokens/hints, calls exact-version engine validation/point derivation, then stores through Daily/provider. Created, existing, conflict, invalid and unavailable outcomes are deliberately mapped to private/no-store responses containing no facts or points. It is not called by `/api/daily/resolve`; the current browser 6D path remains points-v3-only and calls it asynchronously only after owner gameplay persistence and immutable journal freeze. Scopes: `tasks/plans/resolved-at-bat-submission-api.md` and `tasks/plans/points-v4-result-write-api.md`.
