...
The web adapter retains terminal canonical display names in a browser-local `scorecardAnswers` map keyed by pitch number, separate from portable game facts and `DailyShareResult`. Only resolved slots survive restoration. Scorecard and share-card components receive separate inputs. Daily Nine scorecard comparison is also web-only: each completed pitch may asynchronously read the existing exact-pitch comparison aggregate into an ephemeral per-pitch presentation cache, with 0–1 observations withheld and 2+ formatted as ordinary one-decimal `AVG x.x`. These comparison values are not gameplay persistence and are refetched from current aggregates after restore. The engine-formatted spoiler-safe share text remains the portable base; web presentation may decorate each existing pitch line with that same per-AB AVG, never answer names or raw population data. The separate completed-game YOU / AVG / BEAT presentation continues to use the completed-game aggregate. No scoring, server payload, answer-authority or persistence-schema change is required.
