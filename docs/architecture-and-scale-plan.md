...
2. Add Daily Nine same-puzzle/same-ruleset comparison through a portable aggregate/query contract: completion count, average total points, per-at-bat averages/rates, and score distribution. Supabase is the read provider; web/API transports results; React renders them.
3. Settle percentile tie treatment and minimum-sample copy before adding percentile UI. Do not smuggle that product decision into the initial aggregate implementation.
