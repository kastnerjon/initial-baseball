...
3. The portable independent-population comparison contract/service and its server-only Supabase aggregate provider are implemented. PR #199 adds the shared, versioned comparison HTTP shape. PR #200 implements server-side comparison read composition only. PR #201 adds thin GET adapters plus sanitized HTTP mapping and a server-only default-off `DAILY_NINE_COMPARISON_READS_ENABLED` gate; all route responses remain `private, no-store`. The flag stays off in deployed environments until representative performance evidence and an explicit activation checkpoint, so unbenchmarked aggregate reads remain operationally inactive. Engine-derived stored points remain scoring authority; React only renders.
