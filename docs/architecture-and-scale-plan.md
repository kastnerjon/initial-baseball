...
`shared` exports schema-1 submission/result types. Engine `validateDailyCompletedResult` now accepts `points-v3`, `points-v4`, and `classic-inning-v1`, checks the caller-provided puzzle/game and raw facts, and reuses the exact-version engine rules for derivation/completion. `validateDailyAtBatResult` likewise accepts only the two supported Daily Nine result versions, derives awarded points from the exact version, and preserves exact ruleset identity. Output keeps copied, whitelisted facts and drops client totals/answer fields. This portable widening does not widen Supabase codecs/constraints, comparison providers, browser delivery, or the current public default; those remain separate row-G/H work. See `docs/spec/engine.md`, `docs/spec/data-model.md`, and `tasks/plans/points-v4-result-contracts.md`.
...
The Daily comparison math now has a provider-neutral exact-version seam for points-v3 and points-v4. It obtains score ranges from engine rules, accepts signed v4 AB sums, uses score-minus-minimum offset histograms for completed distributions, and calculates strict-lower rates over actual scores rather than array indices. The existing repository/service read port intentionally remains points-v3-only so the current Supabase adapter cannot be treated as v4-capable before its schema/RPC/decoder work. Scope: `tasks/plans/points-v4-comparison-domain.md`.
