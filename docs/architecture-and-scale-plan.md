...
3. The portable independent-population comparison contract/service and its server-only Supabase aggregate provider are implemented. PR #199 adds a shared, versioned comparison HTTP shape with exact population identity and explicit freshness metadata while leaving route/cache/browser policy out. The next concern is the web/server read-only API composition. Measured performance/cache work remains separate. Engine-derived stored points remain scoring authority; React only renders.
