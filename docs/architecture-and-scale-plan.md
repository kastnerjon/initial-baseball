...
Approved target architecture (portable AB contract/service and Supabase provider implemented; collection not activated): Daily Nine receives one immutable observation per terminal AB, independently of completed-game collection. Shared owns transport; engine validates/scores; Daily owns idempotency/read ports; web owns browser coordination, routes and Supabase adapters. Store engine-derived AB points once and aggregate in the database. Keep resolved-AB and completed-game populations/counts separate. Do not merge PR #174's equal-population invariant.
...
The approved browser gate uses a long-lived exclusive Web Lock per stable puzzle/ruleset plus a separate versioned `localStorage` attempt journal/outbox. Browser 6A implements the durable journal/outbox adapter; browser 6B implements exclusive queued ownership and generation fencing; browser 6C now composes both with points-v3 gameplay persistence. A genuinely fresh owned run requires the gameplay key to be truly absent and creates the journal before its first gameplay save; an unreadable/corrupt persisted value fails resolved-AB contribution closed rather than being reclassified as fresh. A pre-rollout points-v3 save without a journal remains completion-only rather than gaining reconstructed AB history. On takeover, the owner reloads durable gameplay/journal state before interaction; immutable journal observations must exactly match the persisted native terminal facts (`pendingAdvance.completedAtBats` when present, otherwise the game state's completed facts), or contribution retires without backfill. Only the owner writes the shared points-v3 save; supported followers are passive and cannot create new completed-result contributions. Reset keeps an empty active attempt but retires any observed attempt before clearing gameplay. A gameplay-save failure retires current contribution locally without blocking gameplay. Classic, older rulesets and unsupported browsers retain existing compatibility persistence and do not gain resolved-AB contribution. Storage events remain notification only; no timeout lease, clock expiry or forced steal exists. Because gameplay and journal remain separate records, 6D still owns the final terminal ordering gameplay save → immutable journal append → asynchronous POST and completed-result attempt-ID reuse. Resolved-AB collection therefore remains inactive through 6C. Scope: `tasks/plans/resolved-at-bat-browser-outbox.md`, `tasks/plans/resolved-at-bat-browser-ownership.md`, `tasks/plans/resolved-at-bat-browser-gameplay-lifecycle.md`, and `tasks/plans/resolved-at-bat-browser-lifecycle.md`.
