...
3. The portable independent-population comparison contract/service and its server-only Supabase aggregate provider are implemented. Hosted migration `20260919164818_create_daily_nine_comparison_reads` installs factual count/sum and score-bucket RPCs as SECURITY INVOKER, service-role-only functions; no browser role receives table/function access. Hosted ACL, behavior, service-role execution and existing-index query plans are verified. The next concern is the read-only API/recovery/freshness contract, then measured performance and UI. Engine-derived stored points remain scoring authority; React only renders.
