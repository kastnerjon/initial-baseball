...
The web adapter retains terminal canonical display names in a browser-local `scorecardAnswers` map keyed by pitch number, separate from portable game facts and `DailyShareResult`. Only resolved slots survive restoration. Daily Nine private scorecard presentation derives the user's personal score for each resolved AB from the persisted `completedAtBats` facts through the existing engine `getDailyAtBatPoints` rule; it does not infer points from the baseball outcome and introduces no new scoring authority or persistence field. Each completed pitch may also asynchronously read the existing exact-pitch comparison aggregate into an ephemeral per-pitch presentation cache, with 0–1 observations withheld and 2+ formatted as ordinary one-decimal `AVG x.x`. Private Daily Nine rows present initials + answer + personal AB points + AVG on one line; Classic rows retain baseball outcomes. The engine-formatted spoiler-safe share text remains the portable base, while web presentation replaces the Daily Nine pitch outcome token with personal AB points and may append the same crowd AVG (`BB: 0 • AVG: 7.0`), never answer names or raw population data. The separate completed-game YOU / AVG / BEAT presentation continues to use the completed-game aggregate. No scoring rule, server payload, answer-authority or persistence-schema change is required.
