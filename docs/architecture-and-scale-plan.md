...
The permanent archive uses the same first-write-wins provider discipline without reusing beta result tables. `public.permanent_daily_issued_puzzles` stores one immutable `permanent-v1` puzzle per permanent Daily identity: stable puzzle ID, date/number, exact ordered nine canonical player IDs, and first issue timestamp. The server-only adapter inserts first and, on a unique conflict, reads the existing identity winner for portable immutable-conflict classification. RLS is enabled with no browser policies; `service_role` has direct `SELECT` and `INSERT` only, with no application update/upsert/delete path. `packages/daily` owns the portable issuance bridge: callers provide an already-resolved permanent identity plus a same-date `scheduled`/`published` editorial lineup, exact slots 1-9 are validated, and the ordered canonical IDs are delegated to the existing immutable issued-puzzle service. `apps/web` owns the server-only persistence composition: one service-role Supabase client constructs the authoritative editorial repository and immutable permanent-issued-puzzle repository; issuance reads the editorial record for the supplied identity date and then delegates all eligibility/order/immutability semantics to the portable service. Missing editorial content fails closed before persistence. This composition still does not know or infer the permanent launch epoch, beta puzzle number/ID, game/ruleset, or automatic schedule. No beta history is imported and no public issuance/archive route exists yet. The Daily layer also owns a separate read-only permanent-puzzle port with lookup by permanent series + Daily number or series + date. The read service validates query shape, allows not-yet-issued puzzles to return null, fails closed if a provider returns a different identity, and does not require a launch epoch to retrieve an already-frozen row. The server-only Supabase adapter implements that port with zero-or-one-row reads over the existing unique `(series_version, daily_number)` and `(series_version, puzzle_date)` keys, reusing the same strict row decoder and query-error mapping as conflict recovery. The read adapter adds no write path, migration, privilege, or launch policy. `apps/web` now also owns the thin server composition: one server Supabase client constructs that provider and the portable Daily read service remains authoritative for query validation and returned identity. This composition does not join canonical baseball facts, build gameplay state, expose HTTP, or infer a launch epoch. A separate web materializer now converts one already-frozen snapshot into the existing `DailyPuzzle` runtime shape without rerunning selection. It resolves each frozen canonical ID through the same gameplay-ready canonical player lookup used by scheduled/published editorial Daily, reuses `createDailyPuzzlePitch` for player identity and hints, restores the frozen canonical ID, and fails closed if any frozen player is unavailable. Permanent ID/date/number/order remain snapshot authority; no game/ruleset, route, launch epoch, persistence, or result behavior is chosen here. The server archive puzzle source now composes those established seams without adding semantics: one server read service handles number/date lookup, null means not issued, and non-null snapshots pass to the materializer. It does not query Supabase directly or duplicate validation/player/hint rules. The server archive runtime now reuses `createDailyRuntimeService` over that source, with the existing progression-token codec/secret and canonical reveal/legacy-ID resolvers. Archive-specific behavior is limited to loading `permanent-v1` by signed/requested date and failing closed when it has not been issued. Existing runtime puzzle-ID/date checks fence a signed archive token to the permanent puzzle loaded by archive endpoints, so no archive token schema is introduced. Public browser exposure remains blocked until archive persistence/session keys are isolated from the current date-based Daily storage. Scopes: `tasks/plans/permanent-daily-issued-puzzle-supabase.md`, `tasks/plans/permanent-daily-issuance.md`, `tasks/plans/permanent-daily-issuance-supabase-composition.md`, `tasks/plans/permanent-daily-issued-puzzle-read.md`, `tasks/plans/permanent-daily-issued-puzzle-supabase-read.md`, `tasks/plans/permanent-daily-issued-puzzle-read-composition.md`, `tasks/plans/permanent-daily-puzzle-materialization.md`, `tasks/plans/permanent-daily-archive-puzzle-source.md`, and `tasks/plans/permanent-daily-archive-runtime.md`.
