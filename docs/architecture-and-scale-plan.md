...
The permanent archive uses the same first-write-wins provider discipline without reusing beta result tables. `public.permanent_daily_issued_puzzles` stores one immutable `permanent-v1` puzzle per permanent Daily identity: stable puzzle ID, date/number, exact ordered nine canonical player IDs, and first issue timestamp. The server-only adapter inserts first and, on a unique conflict, reads the existing identity winner for portable immutable-conflict classification. RLS is enabled with no browser policies; `service_role` has direct `SELECT` and `INSERT` only, with no application update/upsert/delete path. `packages/daily` now owns the portable issuance bridge: callers provide an already-resolved permanent identity plus a same-date `scheduled`/`published` editorial lineup, exact slots 1-9 are validated, and the ordered canonical IDs are delegated to the existing immutable issued-puzzle service. This bridge does not know the permanent launch epoch, beta puzzle number/ID, game/ruleset, or Supabase. No beta history is imported and no public archive route exists yet. Scopes: `tasks/plans/permanent-daily-issued-puzzle-supabase.md` and `tasks/plans/permanent-daily-issuance.md`.
