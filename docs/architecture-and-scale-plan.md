...
5. Treat R7 as the next bounded browser-architecture follow-up: reduce responsibility concentration in `DailyInningGame.tsx` and `useDailyGameplayPersistence.ts` without moving domain rules into React, merging comparison with persistence, or inventing a generic framework. R8 write admission/observability remains a separate server-boundary concern.
