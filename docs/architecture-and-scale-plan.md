...
The approved browser gate uses a long-lived exclusive Web Lock per stable puzzle/ruleset plus a separate versioned `localStorage` attempt journal/outbox. Browser 6A now implements that journal/outbox adapter: one stable points-v3 attempt ID, active/retired contribution state, monotonic generation, immutable schema-1 observations, pending/submitted/conflict/rejected delivery state, exact stored-payload retry, one-tab single-flight delivery and compare-before-write stale-response protection. Same-slot fact disagreement or terminal conflict/rejection retires future contribution rather than rotating identity. The adapter is not wired to gameplay, so collection remains inactive. The future Web Lock serializes the contributing owner; only that owner writes the shared gameplay save or contribution record. A follower is passive while another tab owns the run. After release/crash, a queued owner reloads durable state before continuing and advances the persisted generation. Storage events are notification only; no timeout lease or forced steal is allowed. Because gameplay and journal remain separate records, terminal commit ordering is gameplay save → immutable journal append → asynchronous POST; reset-after-observation ordering is journal retirement → gameplay clear. Hydration mismatches retire contribution without backfill. Missing Web Locks/storage/random-ID support fails closed for new AB collection while leaving existing compatibility gameplay available. Remaining implementation is 6B ownership coordinator, 6C gameplay lifecycle and 6D activation; scope: `tasks/plans/resolved-at-bat-browser-outbox.md` and `tasks/plans/resolved-at-bat-browser-lifecycle.md`.
