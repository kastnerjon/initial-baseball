...
The approved browser gate uses a long-lived exclusive Web Lock per stable puzzle/ruleset plus a separate versioned `localStorage` attempt journal/outbox. Browser 6A implements the durable journal/outbox adapter: one stable points-v3 attempt ID, active/retired contribution state, monotonic generation, immutable schema-1 observations, pending/submitted/conflict/rejected delivery state, exact stored-payload retry, one-tab single-flight delivery and compare-before-write stale-response protection. Browser 6B implements the separate concurrency adapter: deterministic puzzle/ruleset lock naming, exclusive queued ownership, owner/follower/unsupported state, abortable cleanup, durable reload before owner readiness, and generation fencing for an existing journal while the lock is held. A missing journal remains missing; 6B never decides fresh/legacy eligibility or creates an attempt. Storage events are notification only and cannot grant authority. No timeout lease, clock expiry or forced steal exists. Neither adapter is wired to gameplay yet, so collection remains inactive. In 6C only the owner will write the shared gameplay save or contribution record; a follower will remain passive, and fresh/legacy/reset/interrupted-write policy will be composed there. Because gameplay and journal remain separate records, terminal commit ordering remains gameplay save → immutable journal append → asynchronous POST; reset-after-observation ordering remains journal retirement → gameplay clear. Missing Web Locks/storage/random-ID support fails closed for new AB collection while leaving existing compatibility gameplay available. Remaining implementation is 6C gameplay lifecycle and 6D activation; scope: `tasks/plans/resolved-at-bat-browser-outbox.md`, `tasks/plans/resolved-at-bat-browser-ownership.md`, and `tasks/plans/resolved-at-bat-browser-lifecycle.md`.
