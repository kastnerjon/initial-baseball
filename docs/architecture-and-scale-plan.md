...
3. The portable independent-population comparison contract/service is implemented, and the current provider PR adds server-only Supabase aggregate RPCs that return only factual count/sum or score-bucket statistics. The RPCs are SECURITY INVOKER and service-role-only; no browser role receives table/function access. After hosted verification and merge, the next concern is the read-only API/recovery/freshness contract, then measured performance and UI. Engine-derived stored points remain scoring authority; React only renders.
