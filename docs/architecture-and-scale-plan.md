...
The web adapter retains terminal canonical display names in a browser-local `scorecardAnswers` map keyed by pitch number, separate from portable game facts and `DailyShareResult`. Only resolved slots survive restoration. Daily Nine presentation is points-native: the immediate terminal result renders engine-derived `N PTS` rather than the baseball outcome, and each private scorecard row derives the user's personal score from persisted `completedAtBats` facts through the existing engine `getDailyAtBatPoints` rule. Baseball outcome remains an internal/native game fact, not a user-facing Daily Nine score proxy. Each completed pitch may asynchronously read the existing exact-pitch comparison aggregate into an ephemeral per-pitch presentation cache, with 0–1 observations withheld and 2+ formatted as ordinary one-decimal `AVG x.x`. Daily Nine scorecards derive one shared scoring row model of initials + personal AB points + displayable AVG. The in-app scorecard joins the existing browser-local revealed-answer map at presentation time to render Initials / Player / Score / Avg without adding a player-data fetch. The engine-formatted spoiler-safe share text remains the portable base, while web presentation replaces the pitch section with a fixed-width SCORE / AVG table generated from the same scoring rows and never includes answer names. Withheld/unavailable AVG renders `—`; no answer names or raw population data enter share text. The completed-game aggregate powers a single top-line `X PTS • AVG Y.Y` presentation when the AVG is displayable, with BEAT/sample context secondary; the share header uses the same top line. Classic remains baseball-native. No scoring rule, comparison population, server payload, answer-authority or persistence-schema change is required.

## Completed-result and comparison architecture

September 19 review of main `a91edc0`: preserve the boundaries below. R1 outbox ownership lifetime is repaired by binding delivery/retry and acknowledgment writes to one disposable exact attempt/generation owner session, invalidated before lock release. R2 gameplay request lifetime is repaired with a synchronous single-flight controller invalidated by Reset/restore plus persistence-session teardown/owner loss. R3 persistence authorization separates exclusive Web Lock authority from resolved-AB eligibility. R4 persistence re-bootstrap is also repaired: semantic puzzle/ruleset session identity avoids object-identity ownership churn, readiness resets on real session changes, and save/reset require the rendered and live authority/readiness capabilities to match exactly. R5 adds only bounded owner-scoped delivery recovery: a five-second POST deadline plus older-pending retry opportunities at owner acquisition and later durable terminal freezes, with no periodic timer or background drain. R6 isolates untrusted saved-game compatibility decoding from browser storage I/O and makes malformed nested state degrade safely instead of throwing. Evidence, tradeoffs and bounded PR sequence: `docs/engineering/resolved-at-bat-review-2026-09-19.md`.
...
`shared` exports schema-1 submission/result types. Engine `validateDailyCompletedResult` accepts `points-v3`, `points-v4`, and `classic-inning-v1`, checks the caller-provided puzzle/game and raw facts, and reuses the exact-version engine rules for derivation/completion. `validateDailyAtBatResult` accepts exactly the two supported Daily Nine result versions, derives awarded points from the exact version, and preserves exact ruleset identity. Output keeps copied, whitelisted facts and drops client totals/answer fields. G3A/G3B established exact-version v4 Supabase result/comparison seams, and the September 26 redefinition finalized v4 as 4/3/2/1/0.5/0 with exact numeric half-point storage. H1 now widens only the server result-write preflights: the resolved-AB POST routes v3/v4 and the completed-result POST routes v3/v4/Classic, passing the exact routed ruleset into authoritative puzzle loading and engine validation. Browser result creation/delivery, current-Daily save compatibility, shared comparison HTTP/browser acceptance and the public default remain later H work. See `docs/spec/engine.md`, `docs/spec/data-model.md`, `tasks/plans/points-v4-result-contracts.md`, `tasks/plans/points-v4-comparison-supabase-provider.md`, and `tasks/plans/points-v4-result-write-api.md`.
...
The permanent archive uses the same first-write-wins provider discipline without reusing beta result tables. `public.permanent_daily_issued_puzzles` stores one immutable, schema-versioned `permanent-v1` puzzle per permanent Daily identity: stable puzzle ID, date/number, exact ordered nine canonical player IDs, and first issue timestamp; schema v2 also stores the exact public clue layout, labels, initials, and hint values. Existing schema-v1 rows remain readable. The server-only adapter inserts first and, on a unique conflict, reads the existing identity winner for portable immutable-conflict classification. RLS is enabled with no browser policies; `service_role` has direct `SELECT` and `INSERT` only, with no application update/upsert/delete path. `packages/daily` owns the portable issuance bridge: callers provide an already-resolved permanent identity plus a same-date `scheduled`/`published` editorial lineup, exact slots 1-9 are validated, and the ordered canonical IDs and materialized clue snapshot are delegated to the immutable schema-v2 service. `apps/web` owns the server-only persistence composition: one service-role Supabase client constructs the authoritative editorial repository and immutable permanent-issued-puzzle repository; issuance reads the editorial record for the supplied identity date, materializes current public clues through the gameplay adapters, then delegates eligibility/order/immutability semantics to the portable service. Missing editorial content, players, or clues fail closed before persistence. This composition still does not know or infer the permanent launch epoch, beta puzzle number/ID, game/ruleset, or automatic schedule. No beta history is imported and no public issuance/archive route exists yet. The Daily layer also owns a separate read-only permanent-puzzle port with lookup by permanent series + Daily number or series + date. The read service validates query shape, allows not-yet-issued puzzles to return null, fails closed if a provider returns a different identity, and does not require a launch epoch to retrieve an already-frozen row. The server-only Supabase adapter implements that port with zero-or-one-row reads over the existing unique `(series_version, daily_number)` and `(series_version, puzzle_date)` keys, reusing the same strict row decoder and query-error mapping as conflict recovery. The read adapter adds no write path, migration, privilege, or launch policy. `apps/web` owns the thin server composition: one server Supabase client constructs that provider and the portable Daily read service remains authoritative for query validation and returned identity. This composition does not join canonical baseball facts, build gameplay state, expose HTTP, or infer a launch epoch. The web materializer converts the frozen snapshot into the existing `DailyPuzzle` runtime shape without rerunning selection. For schema v1 it retains the original current-clue path; for schema v2 it takes initials, hint layout, labels, and values from the stored clue snapshot while resolving canonical players for answer/reveal identity, and fails closed if the frozen lineup is misaligned or a player is unavailable. Scoring and per-slot outcome mappings remain outside the clue snapshot. The server archive puzzle source composes those seams without adding semantics: one server read service handles number/date lookup, null means not issued, and non-null records pass to the materializer. The server archive runtime reuses `createDailyRuntimeService` over that source, with the existing progression-token codec/secret and canonical reveal/legacy-ID resolvers. Archive-specific behavior is limited to loading `permanent-v1` by signed/requested date and failing closed when it has not been issued; runtime puzzle-ID/date checks fence a signed archive token to the loaded permanent puzzle, so no archive token schema is introduced. Archive browser save/session identity is isolated by PR #253, but the public archive route and UI remain unbuilt. Scopes: `tasks/plans/permanent-daily-issued-puzzle-supabase.md`, `tasks/plans/permanent-daily-issuance.md`, `tasks/plans/permanent-daily-issuance-supabase-composition.md`, `tasks/plans/permanent-daily-issued-puzzle-read.md`, `tasks/plans/permanent-daily-issued-puzzle-supabase-read.md`, `tasks/plans/permanent-daily-issued-puzzle-read-composition.md`, `tasks/plans/permanent-daily-puzzle-materialization.md`, `tasks/plans/permanent-daily-archive-puzzle-source.md`, `tasks/plans/permanent-daily-archive-runtime.md`, and `tasks/plans/permanent-daily-archive-clue-materialization.md`.
...
H2 makes current-Daily gameplay-save storage cutover-safe before browser contribution is widened to points-v4. The historical pre-v4 family keeps `initial-baseball:daily:<date>` and its existing legacy/v1/v2 -> v3 restore compatibility. Points-v4 uses `initial-baseball:daily:ruleset:points-v4:<date>` and only accepts a v4 saved payload; points-v3 rejects v4. Therefore a later v4 play cannot read, overwrite, clear, or reset the historical v3-era value, and a rollback to v3 can still find that untouched save. Classic and permanent archive namespaces are unchanged, and identity-keyed journal/outbox/owner-lock keys are deliberately not translated in H2. Scope: `tasks/plans/points-v4-current-daily-save-isolation.md`.
...
Use the immutable result tables and indexed/read-time population aggregation for the beta; do not add mutable rollups merely to make the asymptotic read cost constant. PR #203's disposable PostgreSQL 17 benchmark exercised the actual functions at 100 / 1,000 / 10,000 target populations. At 10,000, warmed p95 database execution was 1.807 ms for one resolved-AB aggregate and 6.254 ms for completed score buckets, both comfortably inside the provisional 25 ms database budget. The completed synthetic table deliberately made the target 50% of rows, so PostgreSQL chose a sequential scan there and still stayed within budget. This is evidence for the current beta storage choice, not a general traffic-capacity guarantee. Later caching/rollups remain behind read ports: callers receive sufficient statistics and do not know whether the provider scanned raw rows or read a projection. If database execution later becomes the measured bottleneck, prefer a rebuildable transactional projection (AB count + point sum; sparse completed score/count buckets) partitioned by puzzle/ruleset, while keeping immutable raw rows authoritative and scoring outside SQL. The portable comparison contract in `packages/daily` keeps resolved-AB and completed-game reads independent: one slot returns received count plus stored engine-derived point sum, while completed games return score buckets that normalize to the current points-v3 histogram. Daily derives null-or-average values and strict-lower finishers semantics without rescoring. Comparison remains asynchronous and off gameplay's critical path; own outcome/points render immediately. Both comparison GET adapters expose handler-level `Server-Timing` (`daily-comparison-at-bat` / `daily-comparison-completed`) so a real-browser trace can decompose route/server/provider time from browser/network/render time without client telemetry. The first 20-sample-per-route production handler checkpoint found 65 ms / 58 ms medians but 582 ms / 430 ms nearest-rank p95 and greater-than-1.5-second maxima. Because PR #203's isolated raw aggregate remains only 1.807 ms / 6.254 ms p95 at 10,000 target observations, the handler tail does not justify a rollup/cache/index change. Request-local stage timing now preserves the route total and separately measures dynamic server composition, authoritative-puzzle loading and the exact comparison repository/provider call; no global timing context or persistent telemetry is introduced. Production re-sampling localizes the extreme tail to the provider boundary rather than puzzle loading: one completed request measured 2,502 ms total / 2,495 ms provider / 7 ms puzzle. Production `pg_stat_statements` independently keeps the matching PostgREST SQL execution low-millisecond (completed mean 1.921 ms / max 19.696 ms; AB mean 1.594 ms / max 17.332 ms). Do not change aggregate SQL, indexes or rollups from this evidence. Provider sub-timing is now production-sampled across 15 successful reads per route: setup/decode are negligible while the observed multi-second provider tail tracks awaited `client.rpc(...)` (at-bat max 1,495 ms; completed max 2,125 ms). The browser response is to hide that non-critical managed-RPC latency behind gameplay rather than redesign storage: after saved-game hydration, the exact points-v3 slot comparison is prefetched when the AB becomes active, remains undisclosed before terminal resolution, and is projected with engine-derived own points at reveal without restarting solely because own points became known. A separately restored terminal state may fresh-read the current aggregate; historical comparison snapshots are not persisted. The approximately 500 ms p95 trigger-to-visible target remains unverified until real-browser samples are collected. Provider errors remain separate from write/idempotency status. Detailed evidence: `docs/engineering/daily-nine-comparison-performance-2026-09-19.md`. Detailed owning-layer PR sequence and gates: `tasks/plans/resolved-at-bat-comparison.md`. Classic's overall ranking remains unresolved.
...
2. Browser 6A–6D is implemented and normal-path browser-proven; review repairs R1–R6 are implemented. Preserve the exact current-session authority boundary, bounded owner-scoped retry policy, and corruption-safe decode boundary before extending browser persistence.
3. The portable independent-population comparison contract/service and its server-only Supabase aggregate provider are implemented. PR #199 adds the shared, versioned comparison HTTP shape. PR #200 implements server-side comparison read composition only. PR #202 adds thin GET adapters plus sanitized HTTP mapping; all route responses remain `private, no-store`. Representative performance evidence is complete on PR #203 and supports retaining the current raw-read provider. PR #204 made comparison reads default-on while retaining the server-only fail-closed `DAILY_NINE_COMPARISON_READS_DISABLED` emergency switch. Exact merge `178e58cb6fcb8f09ad9ebc3e6ba69cca7a725a01` is production-verified READY as deployment `dpl_F87hcE51cQT6zhoCp8MgGKwCSX38`; both comparison routes returned versioned HTTP 200 live payloads with `private, no-store`, and the comparison-route runtime scan was clean. PR #206 adds a web-only browser comparison transport/decoder over those existing routes: separate at-bat/completed reads, runtime schema-1 decoding, exact server-response identity checks, caller-owned AbortSignal propagation and explicit browser `no-store`. PR #207 adds a separate web-only request-lifecycle controller with independent at-bat/completed channels, same-channel replacement, generation/request fencing, abort on replacement/invalidation, whole-session invalidation, and stale success/error/settled suppression. The at-bat browser checkpoint uses a dedicated identity-keyed React hook and small presentation component over those seams. Its current policy starts the exact slot read after saved-game hydration while the AB is active, keeps the prefetched state hidden during play, and projects that same read with engine-derived own points only at terminal reveal. The completed-game checkpoint adds a separate completed-channel hook that can prefetch from the final pending engine summary and continue through the final-screen transition without restarting on unchanged semantic identity. Whole-game strict-lower finish rate is derived by the existing Daily-domain helper through the narrow browser-safe `@initial-baseball/daily/comparison` subpath, not reimplemented in React or imported through the broader lineup/data barrel. The user's awarded/final points come from engine summaries; comparison I/O remains off gameplay's critical path and outside persistence/result-delivery ownership.
4. At-bat and completed-game comparison React ownership/presentation are implemented with the settled sample thresholds; completed BEAT reuses Daily-owned strict-lower tie semantics and appears only at 20+ completed results. The at-bat read now prefetches on the hydrated active slot while remaining visually hidden until terminal resolution, so terminal own-point projection does not restart the request. September 20 hosted verification confirms the earlier exact-main CI/deployment/RPC baseline; the September 22 provider evidence localizes long tails to awaited RPC rather than PostgreSQL execution. Physical/mobile interaction, trigger-to-visible latency, request-count, and failure-recovery evidence remain open; keep both read paths separate from persistence and result-write delivery. Evidence: `docs/engineering/daily-nine-comparison-hosted-verification-2026-09-20.md`.
5. R7 is structurally complete through R7A–R7C. R8A now bounds actual request-body bytes before JSON parsing on both anonymous result-write routes with a 16 KiB ceiling; keep this transport guard outside scoring, persistence and comparison semantics. R8B is architecturally settled at the Vercel WAF boundary, not in app code: scope only the two anonymous result-write POST routes, count coarsely by IP, start with a generous fixed-window log-only threshold, then enforce 429 only after observed traffic and Preview proof. R8C is implemented at the web HTTP-error boundary: only handled internal 503/500 result-write failures emit a low-cardinality JSON event with route/category/status, while ordinary success/4xx traffic and all raw error/request/user data remain unlogged.
6. Continue outstanding interactive/physical-device QA plus timed editorial rollover/fallback observations without blocking the result pipeline.
7. Build Permanent archive/local-history infrastructure from the future explicit launch Daily #1, then finish broad-launch game/rules/epoch and launch polish.
