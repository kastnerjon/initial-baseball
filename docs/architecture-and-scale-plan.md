...
`shared` exports schema-1 submission/result types. Engine `validateDailyCompletedResult` now accepts `points-v3`, `points-v4`, and `classic-inning-v1`, checks the caller-provided puzzle/game and raw facts, and reuses the exact-version engine rules for derivation/completion. `validateDailyAtBatResult` likewise accepts only the two supported Daily Nine result versions, derives awarded points from the exact version, and preserves exact ruleset identity. Output keeps copied, whitelisted facts and drops client totals/answer fields. G3A now widens only the Supabase result codecs/constraints to match those portable v4 contracts. Comparison providers/RPCs, browser delivery and the current public default remain separate G3B/H work. See `docs/spec/engine.md`, `docs/spec/data-model.md`, and `tasks/plans/points-v4-result-contracts.md`.
...
The Daily comparison math now has a provider-neutral exact-version seam for points-v3 and points-v4. It obtains score ranges from engine rules, accepts signed v4 AB sums, uses score-minus-minimum offset histograms for completed distributions, and calculates strict-lower rates over actual scores rather than array indices. The existing comparison repository/service read port intentionally remains points-v3-only after G3A; only result persistence is v4-capable until G3B widens the comparison RPC/decoder boundary. Scope: `tasks/plans/points-v4-comparison-domain.md`.
