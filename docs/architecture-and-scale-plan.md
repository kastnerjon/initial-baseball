...
3. The portable independent-population comparison contract/service is implemented. Follow `tasks/plans/resolved-at-bat-comparison.md` next with the Supabase aggregate provider as its own PR, then API/recovery and UI. Supabase provides factual aggregates; engine-derived stored points remain scoring authority; React renders.
