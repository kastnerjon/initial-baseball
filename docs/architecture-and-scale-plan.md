...
`shared` exports schema-1 submission/result types. Engine `validateDailyCompletedResult` now accepts `points-v3`, `points-v4`, and `classic-inning-v1`, checks the caller-provided puzzle/game and raw facts, and reuses the exact-version engine rules for derivation/completion. `validateDailyAtBatResult` likewise accepts only the two supported Daily Nine result versions, derives awarded points from the exact version, and preserves exact ruleset identity. Output keeps copied, whitelisted facts and drops client totals/answer fields. G3A/G3B established the Supabase result and comparison-provider seams for exact-version v4. Before public activation, the September 26 v4 redefinition changed that inactive contract to 4/3/2/1/0.5/0, so result storage now uses exact numeric half-point values for v4 and the comparison RPC/provider path carries exact numeric sums/buckets. Browser result delivery, shared comparison HTTP acceptance and the current public default remain H work. See `docs/spec/engine.md`, `docs/spec/data-model.md`, `tasks/plans/points-v4-result-contracts.md`, and `tasks/plans/points-v4-comparison-supabase-provider.md`.
