...
3. The portable independent-population comparison contract/service and its server-only Supabase aggregate provider are implemented. PR #199 adds the shared, versioned comparison HTTP shape. PR #200 implements server-side comparison read composition only. PR #202 adds thin GET adapters plus sanitized HTTP mapping; all route responses remain `private, no-store`. Representative performance evidence is complete on PR #203 and supports retaining the current raw-read provider. PR #204 made comparison reads default-on while retaining the server-only fail-closed `DAILY_NINE_COMPARISON_READS_DISABLED` emergency switch. Exact merge `178e58cb6fcb8f09ad9ebc3e6ba69cca7a725a01` is production-verified READY as deployment `dpl_F87hcE51cQT6zhoCp8MgGKwCSX38`; both comparison routes returned versioned HTTP 200 live payloads with `private, no-store`, and the comparison-route runtime scan was clean. PR #206 adds a web-only browser comparison transport/decoder over those existing routes: separate at-bat/completed reads, runtime schema-1 decoding, exact server-response identity checks, caller-owned AbortSignal propagation and explicit browser `no-store`. It does not add React state or request-generation ownership. The next bounded browser concern is semantic-key/generation stale-response fencing, followed by presentation. Comparison I/O must stay off gameplay's critical path. Engine-derived stored points remain scoring authority; React only renders.
4. Add the bounded browser comparison request-lifecycle controller, then apply settled strict-lower-score tie semantics and initial presentation thresholds; keep both separate from persistence and result-write delivery.
